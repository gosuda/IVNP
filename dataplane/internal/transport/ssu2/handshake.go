...
func NewInitiator(remoteStatic, introKey []byte, destinationID, sourceID uint64) (*Initiator, error) {
	...
	state := dataplanenoise.Initialize(HandshakeProtocol)
	state.MixHash(nil)
...
func deriveDataCiphers(state *dataplanenoise.SymmetricState, localIntro, peerIntro []byte, initiator bool) (*DataCipher, *DataCipher, error) {
