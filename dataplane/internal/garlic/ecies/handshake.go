...
type Initiator struct {
	state        *dataplanenoise.SymmetricState
...
func initializeInitiatorState(cryptoType uint16) (*dataplanenoise.SymmetricState, *HybridInitiator, error) {
	name, err := protocolName(cryptoType)
	if err != nil {
		return nil, nil, err
	}
	state := dataplanenoise.Initialize(name)
...
func initializeResponderState(cryptoType uint16) (*dataplanenoise.SymmetricState, *HybridResponder, error) {
	name, err := protocolName(cryptoType)
	if err != nil {
		return nil, nil, err
	}
...
func mixDH(state *dataplanenoise.SymmetricState, private *ecdh.PrivateKey, public *ecdh.PublicKey) error {
