...
type Packet = dataplanestreaming.Packet

const (
	HeaderLen       = dataplanestreaming.HeaderLen
	MaxPacketSize   = dataplanestreaming.MaxPacketSize
	InitialWindow   = dataplanestreaming.InitialWindow
	MaxWindow       = dataplanestreaming.MaxWindow
	FlagSynchronize = dataplanestreaming.FlagSynchronize
	FlagClose       = dataplanestreaming.FlagClose
	FlagReset       = dataplanestreaming.FlagReset
	FlagNoACK       = dataplanestreaming.FlagNoACK
)
const (
...
func (n *TunnelNetwork) maintain() {
	defer n.wg.Done()
	defer func() {
		for _, queue := range n.deliveryQueues {
			close(queue)
		}
	}()
...
func (r sendRequest) finish(err error) {
	...
	r.lease.release()
	if err != nil && r.abortOnFailure {
		r.connection.abort(false)
	}
}
...
func (c *tunnelConn) handle(ctx context.Context, delivery Delivery, packet Packet) error {
	...
	for index, resend := range fastRetransmit {
		if err := c.queueProtocolOwned(resend.wire, resend.lease); err != nil {
...
func (c *tunnelConn) sendSynchronize(ctx context.Context, originator bool) error {
	...
	if originator {
		err = c.sendWireOwned(ctx, wire, lease)
	} else {
		err = c.queueProtocolOwned(wire, lease)
	}
	if err != nil {
		c.mu.Lock()
...
func (c *tunnelConn) sendACK() error {
	c.mu.Lock()
