...
const (
	...
	TunnelPayloadLen  = foundation.I2NPTunnelDataPayloadLen
	tunnelIVLen       = 16
	tunnelChecksumLen = 4
...
func (g *Gateway) Encode(tunnelID uint32, blocks []Block, dst *packet.Buffer) error {
	if tunnelID == 0 {
		return foundation.I2NPErrInvalidTunnelID
	}
	if dst == nil {
		return ErrGatewayOutput
	}
	available, ok := dst.AvailablePayload()
	if !ok || available < foundation.I2NPTunnelDataMessageLen {
		return ErrGatewayOutput
	}
	...
	out, ok := dst.Append(foundation.I2NPTunnelDataMessageLen)
	if !ok {
		return ErrGatewayOutput
	}
...
func (g *Gateway) Fragment(tunnelID uint32, block Block, dst []*packet.Buffer) (int, error) {
	if tunnelID == 0 {
		return 0, foundation.I2NPErrInvalidTunnelID
	}
...
func (e *Endpoint) Parse(payload []byte, out []Block, nowMillis uint64) (int, error) {
	message, err := foundation.I2NPParseTunnelData(payload)
	if err != nil {
		return 0, err
	}
	data := message.Data
