...
func TestGatewayRoundTripsDeliveryInstructions(t *testing.T) {
	...
	defer buf.Release()
	g := NewGateway(bytes.NewReader(bytes.Repeat([]byte{0x7f}, foundation.I2NPTunnelDataMessageLen)))
...
func TestGatewayFragmentsAndReassembles(t *testing.T) {
	...
	g := NewGateway(bytes.NewReader(bytes.Repeat([]byte{0x7f}, 4*foundation.I2NPTunnelDataMessageLen)))
...
func TestGatewayRejectsTruncatedAndOversizedBlocks(t *testing.T) {
	g := NewGateway(bytes.NewReader(bytes.Repeat([]byte{1}, foundation.I2NPTunnelDataMessageLen)))
	buf := testPacketBuffer(t)
	defer buf.Release()
	...
	if _, err := endpoint.Parse(make([]byte, foundation.I2NPTunnelDataMessageLen-1), nil, 0); err == nil {
...
func TestGatewayRejectsChecksumMismatch(t *testing.T) {
	buf := testPacketBuffer(t)
	defer buf.Release()
	if err := NewGateway(bytes.NewReader(bytes.Repeat([]byte{0x7f}, foundation.I2NPTunnelDataMessageLen))).Encode(
