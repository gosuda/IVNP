...
type ntcp2InitiatorFactory func([]byte) (*dataplanentcp2.Initiator, error)
...
func (m *NTCP2Manager) Close() error {
	...
	m.close.Do(func() {
		...
		listener := m.listener
		cancel := m.cancel
		sessions := make([]*dataplanentcp2.Session, 0, len(m.sessions))
...
func (m *NTCP2Manager) EnsureSession(ctx context.Context, peer foundation.Hash) error {
	...
	stopCancel := context.AfterFunc(m.ctx, cancelSetup)
	defer stopCancel()
	defer cancelSetup()
	...
	err := m.openOutbound(setupCtx, peer)
	if m.session(peer) != nil {
		err = nil
	}
	result := err
...
func (m *NTCP2Manager) HasSession(peer foundation.Hash) bool {
	return m != nil && m.contextErr() == nil && m.session(peer) != nil
}
...
func (m *NTCP2Manager) Send(ctx context.Context, peer foundation.Hash, message foundation.I2NPMessage) error {
...
func (m *NTCP2Manager) writeI2NP(session *dataplanentcp2.Session, message foundation.I2NPMessage) error {
	...
	if err == nil && m.metrics != nil {
		m.metrics.AddTransportSentBytes(uint64(dataplanentcp2.BlockHeaderLen + foundation.I2NPTransportHeaderLen + len(message.Payload)))
	}
	return err
}
...
func (m *NTCP2Manager) openOutbound(ctx context.Context, peer foundation.Hash) error {
	...
	if len(payload)+64 > dataplanentcp2.MaxSessionRequestLen || len(payload)+dataplanentcp2.FrameTagLen > 0xffff {
		return ErrNTCP2Peer
	}
	...
	request, err := initiator.BuildSessionRequest(make([]byte, dataplanentcp2.SessionRequestCiphertextLen+len(padding)), peer[:], remote.iv[:], padding, dataplanentcp2.SessionRequestOptions{
		NetworkID:           m.networkID,
		Version:             2,
		PaddingLength:       uint16(len(padding)),
		Message3Part2Length: uint16(len(payload) + dataplanentcp2.FrameTagLen),
...
func (m *NTCP2Manager) readSession(peer foundation.Hash, session *dataplanentcp2.Session) {
	...
	plaintext := make([]byte, dataplanentcp2.MaxPlaintextFrame)
	for {
...
func (m *NTCP2Manager) handleNTCP2Frame(peer foundation.Hash, frame []byte) bool {
	iterator := dataplanentcp2.NewBlockIterator(frame)
	terminated := false
	for {
		...
		switch block.Type {
		case dataplanentcp2.BlockDateTime:
			if len(block.Data) != 4 {
				return false
			}
		case dataplanentcp2.BlockI2NP:
			if !m.handleNTCP2I2NPBlock(peer, block.Data) {
				return false
			}
		case dataplanentcp2.BlockRouterInfo:
			if !m.handleNTCP2RouterInfoBlock(peer, block.Data) {
				return false
			}
		case dataplanentcp2.BlockTermination:
			terminated = true
		}
	}
}
...
func (m *NTCP2Manager) handleNTCP2RouterInfoBlock(peer foundation.Hash, data []byte) bool {
	...
	return m.peers.AdmitRouterInfo(info, uint64(now.UnixMilli())) == nil
}

func (m *NTCP2Manager) localHandshakePayload(local TransportLocalInfo) ([]byte, error) {
	if local == nil {
		return nil, ErrNTCP2ManagerConfig
	}
	info := local.Snapshot()
	...
	if len(info.Bytes())+1 > dataplanentcp2.MaxBlockData {
		return nil, ErrNTCP2Peer
	}
	payload := make([]byte, dataplanentcp2.BlockHeaderLen+1+len(info.Bytes()))
	payload[0] = dataplanentcp2.BlockRouterInfo
...
func validateNTCP2HandshakePayload(payload, static []byte) (foundation.NetworkDatabaseRouterInfo, error) {
	iterator := dataplanentcp2.NewBlockIterator(payload)
	first, ok, err := iterator.Next()
	if err != nil || !ok || first.Type != dataplanentcp2.BlockRouterInfo || len(first.Data) < 2 || first.Data[0]&^byte(1) != 0 {
		return foundation.NetworkDatabaseRouterInfo{}, ErrNTCP2Peer
	}
	info, err := foundation.NetworkDatabaseParseRouterInfo(first.Data[1:])
	if err != nil {
	...
	for {
		...
		if block.Type == dataplanentcp2.BlockOptions && len(block.Data) >= 12 {
			continue
		}
		if block.Type == dataplanentcp2.BlockPadding {
			continue
		}
		return foundation.NetworkDatabaseRouterInfo{}, ErrNTCP2Peer
	}
...
func hasNTCP2Static(info foundation.NetworkDatabaseRouterInfo, static []byte) bool {
...
func hasNTCP2LocalAddress(info foundation.NetworkDatabaseRouterInfo, static, iv []byte) bool {
	addresses := info.Addresses()
	for {
		address, ok, err := addresses.Next()
		if err != nil || !ok {
...
func selectNTCP2Address(info foundation.NetworkDatabaseRouterInfo) (ntcp2Address, error) {
	return selectNTCP2AddressForNetwork(info, ntcp2AddressAny)
}

func selectNTCP2AddressForNetwork(info foundation.NetworkDatabaseRouterInfo, mode ntcp2AddressMode) (ntcp2Address, error) {
...
func marshalNTCP2I2NP(message foundation.I2NPMessage) ([]byte, error) {
	encoded := make([]byte, foundation.I2NPTransportHeaderLen+len(message.Payload))
...
func marshalNTCP2I2NPTo(dst []byte, message foundation.I2NPMessage) error {
	...
	encodedLen := foundation.I2NPTransportHeaderLen + len(message.Payload)
	...
	copy(dst[foundation.I2NPTransportHeaderLen:encodedLen], message.Payload)
	return nil
}

func decodeNTCP2I2NP(data []byte) (foundation.I2NPMessage, error) {
	...
	return foundation.I2NPMessage{
		...
		Payload: append([]byte(nil), data[foundation.I2NPTransportHeaderLen:]...),
	}, nil
}

func writeNTCP2I2NP(session *dataplanentcp2.Session, message foundation.I2NPMessage) error {
	if len(message.Payload) > foundation.I2NPI2PDMaxPayload {
		return foundation.I2NPErrPayloadTooLarge
	}
	plaintextLen := dataplanentcp2.BlockHeaderLen + foundation.I2NPTransportHeaderLen + len(message.Payload)
	...
	plaintext[0] = dataplanentcp2.BlockI2NP
	binary.BigEndian.PutUint16(plaintext[1:3], uint16(len(plaintext)-dataplanentcp2.BlockHeaderLen))
	if err := marshalNTCP2I2NPTo(plaintext[dataplanentcp2.BlockHeaderLen:], message); err != nil {
		return err
	}
...
func (m *NTCP2Manager) session(peer foundation.Hash) *dataplanentcp2.Session {
	m.mu.RLock()
