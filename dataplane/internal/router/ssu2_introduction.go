...
func (m *SSU2Manager) ssu2KeysForPeer(peer foundation.Hash) (ssu2PeerAddress, error) {
	...
	return selectSSU2Keys(info)
}

func (m *SSU2Manager) routerInfo(peer foundation.Hash) (foundation.NetworkDatabaseRouterInfo, bool) {
	if m.peers == nil {
		return foundation.NetworkDatabaseRouterInfo{}, false
	}
	return m.peers.RouterInfo(peer)
}
...
func (m *SSU2Manager) handleRelayRequest(alice *ssu2TransportSession, request dataplanessu2.RelayRequest) {
	m.mu.RLock()
	...
	if !found || charlie == nil || !m.sessionActive(charlie) {
		m.sendRelayResponse(alice, localHash, dataplanessu2.RelayResponse{Code: ssu2RelayRejectTagNotFound, Nonce: request.Nonce, Timestamp: uint32(m.now().Unix())})
		return
	}
	aliceInfo, known := m.routerInfo(alice.peer)
	input, err := dataplanessu2.RelayRequestSignatureInput(nil, localHash[:], charliePeer[:], request)
	if err != nil || !known || !m.relayTimestampValid(request.Timestamp) {
		m.sendRelayResponse(alice, localHash, dataplanessu2.RelayResponse{Code: ssu2RelayRejectSignature, Nonce: request.Nonce, Timestamp: uint32(m.now().Unix())})
		return
	}
	...
	if len(m.relayForwards) >= m.maxPending {
		m.mu.Unlock()
		m.sendRelayResponse(alice, localHash, dataplanessu2.RelayResponse{Code: ssu2RelayRejectLimit, Nonce: request.Nonce, Timestamp: uint32(m.now().Unix())})
		return
	}
	jobs, ctx := m.relayStoreJobs, m.ctx
	if jobs == nil || ctx == nil || !m.runningLocked() {
		m.mu.Unlock()
		m.sendRelayResponse(alice, localHash, dataplanessu2.RelayResponse{Code: ssu2RelayRejectGeneralFailure, Nonce: request.Nonce, Timestamp: uint32(m.now().Unix())})
		return
	}
...
func (m *SSU2Manager) forwardRelayStore(job ssu2RelayStoreJob) {
	...
	if err == nil {
		var intro []byte
		intro, err = dataplanessu2.MarshalRelayIntroBlock(nil, dataplanessu2.RelayIntro{AliceHash: [32]byte(job.alice.peer), Request: job.request})
		if err == nil {
			err = m.sendSessionData(job.charlie, intro, false)
		}
...
func (m *SSU2Manager) failRelayForward(job ssu2RelayStoreJob) {
	...
	m.mu.Unlock()
	if active {
		m.sendRelayResponse(job.alice, job.localHash, dataplanessu2.RelayResponse{Code: ssu2RelayRejectGeneralFailure, Nonce: job.request.Nonce, Timestamp: uint32(m.now().Unix())})
	}
}

func (m *SSU2Manager) handleRelayIntro(bob *ssu2TransportSession, intro dataplanessu2.RelayIntro) {
	m.handleRelayIntroAttempt(bob, intro, 0)
}

func (m *SSU2Manager) handleRelayIntroAttempt(bob *ssu2TransportSession, intro dataplanessu2.RelayIntro, attempt uint8) {
	bindings := m.currentBindings()
	if bindings.LocalInfo == nil {
		return
	}
	localHash := bindings.LocalInfo.Hash()
	response := dataplanessu2.RelayResponse{Code: ssu2RelayRejectAliceUnknown, Nonce: intro.Request.Nonce, Timestamp: uint32(m.now().Unix())}
...
func (m *SSU2Manager) processRelayIntro(bob *ssu2TransportSession, intro dataplanessu2.RelayIntro, localHash foundation.Hash, aliceInfo foundation.NetworkDatabaseRouterInfo) (dataplanessu2.RelayResponse, bool) {
	response := dataplanessu2.RelayResponse{Code: ssu2RelayRejectSignature, Nonce: intro.Request.Nonce, Timestamp: uint32(m.now().Unix())}
	input, err := dataplanessu2.RelayRequestSignatureInput(nil, bob.peer[:], localHash[:], intro.Request)
	if err != nil {
	...
	destinationID, sourceID := dataplanessu2.RelayConnectionIDs(intro.Request.Nonce)
	...
	response = dataplanessu2.RelayResponse{
	...
	block, err := dataplanessu2.MarshalRelayResponseBlock(nil, response)
	if err != nil || m.sendSessionData(bob, block, false) != nil {
		return response, true
	}
...
func (m *SSU2Manager) deferRelayIntro(bob *ssu2TransportSession, intro dataplanessu2.RelayIntro, attempt uint8) bool {
	m.mu.Lock()
...
func (m *SSU2Manager) handleRelayResponse(from *ssu2TransportSession, response dataplanessu2.RelayResponse) {
	m.mu.RLock()
...
func (m *SSU2Manager) forwardRelayResponse(from *ssu2TransportSession, response dataplanessu2.RelayResponse, data []byte) {
	m.mu.Lock()
	...
	payload, err := dataplanessu2.MarshalBlock(nil, dataplanessu2.BlockRelayResponse, data)
	if err == nil {
		_ = m.sendSessionData(forward.alice, payload, false)
	}
}

func (m *SSU2Manager) acceptRelayResponse(response dataplanessu2.RelayResponse, relay *ssu2RelayRequest) {
	if response.Code != 0 {
		m.mu.Lock()
	...
	input, err := dataplanessu2.RelayResponseSignatureInput(nil, relay.introducer[:], response)
...
func (m *SSU2Manager) startIntroducedOutbound(response dataplanessu2.RelayResponse, relay *ssu2RelayRequest) {
	destinationID, sourceID := dataplanessu2.RelayConnectionIDs(response.Nonce)
	m.mu.Lock()
...
func (m *SSU2Manager) sendRelayResponse(session *ssu2TransportSession, bobHash foundation.Hash, response dataplanessu2.RelayResponse) {
	if !m.signRelayResponse(bobHash, &response) {
		return
	}
	payload, err := dataplanessu2.MarshalRelayResponseBlock(nil, response)
	if err == nil {
		_ = m.sendSessionData(session, payload, false)
	}
}

func (m *SSU2Manager) signRelayResponse(bobHash foundation.Hash, response *dataplanessu2.RelayResponse) bool {
	input, err := dataplanessu2.RelayResponseSignatureInput(nil, bobHash[:], *response)
	if err != nil {
		return false
	}
...
func (m *SSU2Manager) sendHolePunch(aliceAddress ssu2PeerAddress, remote netip.AddrPort, response dataplanessu2.RelayResponse) {
...
func (m *SSU2Manager) holePunchPayload(response dataplanessu2.RelayResponse) ([]byte, error) {
	...
	payload, err = dataplanessu2.MarshalBlock(payload, dataplanessu2.BlockAddress, data)
	if err != nil {
		return nil, err
	}
	return dataplanessu2.MarshalRelayResponseBlock(payload, response)
}

func (m *SSU2Manager) handleHolePunch(header dataplanessu2.LongHeader, payload []byte, _ net.Addr) {
	iterator := dataplanessu2.NewBlockIterator(payload)
	var response dataplanessu2.RelayResponse
	for {
		block, ok, err := iterator.Next()
		if err != nil || !ok {
			return
		}
		if block.Type != dataplanessu2.BlockRelayResponse {
			continue
		}
...
func ssu2RouterInfoStore(info foundation.NetworkDatabaseRouterInfo, now time.Time) (foundation.I2NPMessage, error) {
...
func (m *SSU2Manager) cachedSSU2RouterInfoStore(info foundation.NetworkDatabaseRouterInfo, now time.Time) (foundation.I2NPMessage, error) {
	hash := info.Hash()
	raw := info.Bytes()
...
func newSSU2RouterInfoStoreSnapshot(info foundation.NetworkDatabaseRouterInfo) (ssu2RouterInfoStoreSnapshot, error) {
	raw := info.Bytes()
	if len(raw) == 0 || len(raw) > foundation.NetworkDatabaseMaxRouterInfoBytes {
	...
	if compressed.Len() == 0 || compressed.Len() > foundation.NetworkDatabaseMaxRouterInfoBytes || compressed.Len() > 0xffff {
		return ssu2RouterInfoStoreSnapshot{}, ErrSSU2Peer
	}
...
func (snapshot ssu2RouterInfoStoreSnapshot) message(now time.Time) (foundation.I2NPMessage, error) {
	payload := make([]byte, 39+len(snapshot.compressed))
	copy(payload[:32], snapshot.hash[:])
	payload[32] = byte(foundation.I2NPStoreRouterInfo)
	binary.BigEndian.PutUint16(payload[37:39], uint16(len(snapshot.compressed)))
	copy(payload[39:], snapshot.compressed)
	if _, err := foundation.I2NPParseDatabaseStore(payload); err != nil {
		return foundation.I2NPMessage{}, err
	}
	...
	return foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPDatabaseStore, ID: id, Expiration: uint64(now.Add(time.Minute).UnixMilli())}, Payload: payload}, nil
}

func selectSSU2Keys(info foundation.NetworkDatabaseRouterInfo) (ssu2PeerAddress, error) {
...
func selectSSU2Introducers(info foundation.NetworkDatabaseRouterInfo, now uint64) []ssu2IntroducerCandidate {
	addresses := info.Addresses()
