...
func TestStableRouterSSU2Interop(t *testing.T) {
	...
	if err = database.AdmitRouterInfo(peer, uint64(time.Now().UnixMilli())); err != nil {
		_ = connection.Close()
	...
	if err = manager.Start(ctx, TransportBindings{
		...
		HandleI2NPContext: func(_ context.Context, source foundation.Hash, message foundation.I2NPMessage, _ uint64, _ bool) error {
			select {
	...
	if _, err = foundation.I2NPParseDatabaseLookup(lookup); err != nil {
		t.Fatalf("build stable-router DatabaseLookup: %v", err)
	}
	message := foundation.I2NPMessage{
		Header:  foundation.I2NPHeader{Type: foundation.I2NPDatabaseLookup, ID: 1, Expiration: uint64(time.Now().Add(time.Minute).UnixMilli())},
		Payload: lookup,
	}
	if err = manager.EnsureSession(ctx, expectedHash); err != nil {
		t.Fatalf("authenticate stable-router SSU2 session: %v", err)
	}
...
func requireStableSSU2RoundTrip(t *testing.T, received <-chan stableSSU2Reply, peer foundation.Hash) {
	...
	case reply := <-received:
		...
		if reply.message.Header.Type != foundation.I2NPDatabaseStore {
			t.Fatalf("stable-router reply type = %d, want DatabaseStore", reply.message.Header.Type)
		}
		store, err := foundation.I2NPParseDatabaseStore(reply.message.Payload)
		if err != nil {
			t.Fatalf("parse stable-router DatabaseStore: %v", err)
		}
		if store.Type != foundation.I2NPStoreRouterInfo || store.Key != peer {
