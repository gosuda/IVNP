...
var _ stream.StreamNetwork = (*DestinationManager)(nil)
...
func TestDestinationManagerOwnsStreamSessions(t *testing.T) {
	...
	leftSession, err := left.Create(DestinationSessionConfig{Default: true, Streaming: dataplanestreamingtunnel.TunnelNetworkConfig{Destination: leftDestination, Sender: fabric}})
	if err != nil {
		t.Fatal(err)
	}
	rightSession, err := right.Create(DestinationSessionConfig{Default: true, Streaming: dataplanestreamingtunnel.TunnelNetworkConfig{Destination: rightDestination, Sender: fabric}})
	if err != nil {
		t.Fatal(err)
	}
	fabric.mu.Lock()
	...
	if err := right.HandleStreaming(context.Background(), dataplanestreamingtunnel.Delivery{To: rightSession.Hash(), Protocol: dataplanestreamingtunnel.ProtocolStreaming}); err != ErrDestinationNotFound {
...
func TestDestinationSessionDeliversSelfMessagesLocally(t *testing.T) {
	...
	for _, protocol := range []uint8{17, 18} {
		...
		for sequence := range 4 {
			...
			if sendErr := session.SendMessage(context.Background(), dataplanestreamingtunnel.Delivery{
...
func (f *destinationFabric) SendTunnel(ctx context.Context, delivery dataplanestreamingtunnel.Delivery) error {
	f.mu.Lock()
