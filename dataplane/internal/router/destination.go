...
type DestinationSession struct {
	...
	stream  *dataplanestreamingtunnel.TunnelNetwork
	sender  dataplanestreamingtunnel.TunnelSender
	release func()
	once    sync.Once
...
func (m *DestinationManager) Create(config DestinationSessionConfig) (*DestinationSession, error) {
	...
	stream, err := dataplanestreamingtunnel.NewTunnelNetwork(config.Streaming)
	if err != nil {
		return nil, err
	}
...
func (m *DestinationManager) HandleMessage(ctx context.Context, delivery dataplanestreamingtunnel.Delivery) error {
	if m == nil {
		return ErrDestinationNotFound
	}
	m.mu.RLock()
...
func (m *DestinationManager) deliverLocalMessage(session *DestinationSession, delivery dataplanestreamingtunnel.Delivery) error {
...
func (m *DestinationManager) HandleStreaming(ctx context.Context, delivery dataplanestreamingtunnel.Delivery) error {
...
func (s *DestinationSession) StreamingStats() dataplanestreamingtunnel.NetworkStats {
	if s == nil {
		return dataplanestreamingtunnel.NetworkStats{}
	}
...
func (s *DestinationSession) SendMessage(ctx context.Context, delivery dataplanestreamingtunnel.Delivery) error {
...
func (s *DestinationSession) SubscribeBounded(route destination.DestinationRoute, capacity int, maxBytes int64, shared destination.ByteBudget) (destination.MessageSubscription, error) {
	if s == nil || route.Protocol == dataplanestreamingtunnel.ProtocolStreaming || capacity < 1 || maxBytes < 0 {
...
func (s *DestinationSession) deliver(delivery dataplanestreamingtunnel.Delivery) error {
	s.routeMu.RLock()
...
func (s *destinationSubscription) enqueue(delivery dataplanestreamingtunnel.Delivery) error {
