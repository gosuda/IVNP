...
const (
	...
	kademliaRangeCount    = foundation.HashLength * 8 * kademliaBFactor
...
func kademliaRange(local, peer foundation.Hash) int {
	leading := 0
	for index := range foundation.HashLength {
	...
	if leading == foundation.HashLength*8 {
		return -1
	}
	highBit := foundation.HashLength*8 - 1 - leading
...
func (s *kBucketSet) add(local, peer foundation.Hash, now uint64) bool {
...
func (s *kBucketSet) split(index int, local foundation.Hash) {
	for {
		...
		first := kBucket{begin: uint16(start), end: uint16(secondStart - 1), members: make([]foundation.Hash, 0, len(original.members)), lastChanged: original.lastChanged}
		second := kBucket{begin: uint16(secondStart), end: uint16(end), members: make([]foundation.Hash, 0, len(original.members)), lastChanged: original.lastChanged}
...
func (s *kBucketSet) redistribute(bucket *kBucket, member foundation.Hash) {
	if bucket.begin == bucket.end && len(bucket.members) >= s.capacity {
		return
	}
...
func (s *kBucketSet) remove(local, peer foundation.Hash) bool {
	rangeNumber := kademliaRange(local, peer)
	if rangeNumber < 0 {
		return false
	}
