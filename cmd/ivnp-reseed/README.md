...
[I2P Network]
      ...
      ▼
┌─────────────────────────────────────────────────────────────┐
│ Embedded Client Router (Port 0, Unadvertised, SSU2/NTCP2)   │
│ - NetDB Table (In-Memory Routing Info)                      │
│ - Tuned Exploratory Tunnels (Pool Cap 32, Pending Cap 128)  │
│ - DHT Tree Exploration (Targets empty/sparse 256 K-Buckets) │
└─────────────────────────────┬───────────────────────────────┘
                              │ ActiveCrawler.HarvestRefs()
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│ - Real-time 256 K-Bucket distribution tracking              │
└─────────────────────────────┬───────────────────────────────┘
                              │ Prober.ProbeAll() (60 probes/sec, 48 workers)
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
1. **Active DHT Prefix Tree Exploration**: Rather than passively relying on local exploratory tunnels, the crawler analyzes the 256-bucket distribution in the peer store every pass. Buckets with low or zero coverage are actively targeted with synthetic DHT exploration queries (`TreeExplore`) dispatched to the embedded router's exploratory subsystem.
2. **Dual-Transport Handshakes & SSU2 Awareness**: Supports both SSU2 and NTCP2 transports. The prober targets verified TCP ports for NTCP2 routers and recognizes recent router-level SSU2 inbound handshakes without issuing false-negative TCP dials.
3. **High-Throughput Outbound Probing**: Outbound reachability checks are throttled by a token bucket rate limiter at `60.0 queries/sec` (burst `120.0`) with 48 concurrent workers. Probing uses differentiated cooldowns: 3 minutes for reachable peers, 90 seconds for failed peers, and immediate probing (0s) for unprobed newcomers.
4. **Sybil & Eclipse Mitigation**: Peer selection enforces a hard limit of 1 router per IPv4 `/24` subnet and 1 router per declared `family`, distributed uniformly across all 256 K-Buckets.
5. **Memory Bound**: The in-memory peer store caps storage at 5,000 entries. When saturated, lowest-scoring peers are evicted via min-heap/linear scan before accepting newcomers.
