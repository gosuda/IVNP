...
func (s *RouterInfoStore) Load(nowMillis uint64) (RouterInfoLoadResult, error) {
	...
	file, _, err := fsstore.OpenRegular(s.path)
	...
	defer file.Close()
	data, err := fsstore.ReadBoundedFile(file, int64(s.MaxBytes()))
	if err != nil {
		return RouterInfoLoadResult{}, err
	}
...
func (s *RouterInfoStore) Save() error {
	...
	if err := fsstore.WriteAtomic(s.path, data, 0o600, s.MaxBytes()); err != nil {
		return err
	}
