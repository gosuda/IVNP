...
func TestNewSessionRoundTripAndTamperRejection(t *testing.T) {
	...
	if len(packet) <= cryptx.ElGamalCiphertextSize || packet[0] != 0 || packet[257] != 0 {
		t.Fatalf("new-session layout = %d bytes", len(packet))
	}
	payload, tags, receivedKey, err := ReceiveNew(make([]byte, len(packet)-cryptx.ElGamalCiphertextSize), packet, private)
	...
	badElGamal[0] = 1
	if _, _, _, err := ReceiveNew(make([]byte, len(packet)-cryptx.ElGamalCiphertextSize), badElGamal, private); err == nil {
		t.Fatal("accepted malformed ElGamal prefix")
	}
	badBody := append([]byte(nil), packet...)
	badBody[cryptx.ElGamalCiphertextSize] ^= 1
	if _, _, _, err := ReceiveNew(make([]byte, len(packet)-cryptx.ElGamalCiphertextSize), badBody, private); err == nil {
