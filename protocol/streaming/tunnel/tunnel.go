...
// Delivery is one authenticated Streaming payload after Garlic/tunnel routing
// has selected the local destination. Payload is borrowed by SendTunnel only;
// implementations which queue it must copy it before returning.
