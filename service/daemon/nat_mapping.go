...
type natPMPClient interface {
	...
	Unmap(context.Context, natpmp.Mapping) error
}

type upnpClient interface {
	Discover(context.Context) ([]upnp.DiscoveryResponse, error)
	Describe(context.Context, *url.URL) (upnp.Gateway, error)
	ExternalAddress(context.Context, upnp.Gateway) (netip.Addr, error)
	AddPortMapping(context.Context, upnp.Gateway, upnp.PortMapping) error
	DeletePortMapping(context.Context, upnp.Gateway, string, uint16, string) error
}
...
func newNATMappingPublisher(base staticAddressPublisher, ntcp2, ssu2 autoTransportConfig, natPMPEndpoint netip.AddrPort, upnpEndpoint string, localInfo *router.LocalRouterInfo, logger *slog.Logger) router.AddressPublisher {
	if !ntcp2.automatic && !ssu2.automatic {
		return base
	}
	publisher := &natMappingPublisher{
		base:           append([]router.PublishedAddress(nil), base...),
		config:         configTransports{ntcp2: ntcp2, ssu2: ssu2},
		localInfo:      localInfo,
		logger:         logger,
		upnp:           new(upnp.Client),
	...
	publisher.newNATPMP = func(endpoint netip.AddrPort) natPMPClient {
		client := natpmp.NewClient(endpoint.Addr())
...
func (p *natMappingPublisher) Addresses(context.Context) ([]router.PublishedAddress, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.addressesLocked(), nil
}

func (p *natMappingPublisher) AddressesForSockets(ctx context.Context, ntcp2 net.Listener, ssu2 net.PacketConn) ([]router.PublishedAddress, error) {
	p.mu.Lock()
	...
	p.mu.Unlock()
	if reachable {
		p.localInfo.SetReachability(router.ReachabilityReachable)
	} else {
		p.localInfo.SetReachability(router.ReachabilityFirewalled)
	}
...
func (p *natMappingPublisher) attemptNATPMP(ctx context.Context, spec natMappingSpec, externalPort uint16) (*activeNATMapping, error) {
	...
	if p.natPMPEndpoint.IsValid() {
		...
		for i := range gateways {
			candidates[i] = netip.AddrPortFrom(gateways[i], natpmp.DefaultPort)
		}
	}
	if len(candidates) == 0 {
...
func (p *natMappingPublisher) mapNATPMP(ctx context.Context, client natPMPClient, gateway, public netip.Addr, spec natMappingSpec, externalPort uint16) (*activeNATMapping, error) {
	...
	mapping, err := client.Map(ctx, natpmp.MappingRequest{Protocol: protocol, InternalPort: spec.internalPort, ExternalPort: externalPort, Lifetime: natRequestedLifetime})
	if err != nil {
		return nil, err
	}
...
func (p *natMappingPublisher) upnpGateway(ctx context.Context) (upnp.Gateway, error) {
	p.mu.Lock()
	cached := p.cachedUPnP
	p.mu.Unlock()
	...
	p.mu.Unlock()
	if unavailable {
		return upnp.Gateway{}, errUPnPDiscoveryUnavailable
	}
...
func (p *natMappingPublisher) renew(ctx context.Context, current *activeNATMapping) (*activeNATMapping, error) {
	...
	mapping := upnp.PortMapping{ExternalPort: current.externalPort, Protocol: current.spec.protocol, InternalPort: current.spec.internalPort, InternalClient: current.internalIP.String(), Enabled: true, Description: "ivnp " + strings.ToLower(current.spec.transport), LeaseDuration: uint32(natRequestedLifetime / time.Second)}
...
func (p *natMappingPublisher) addressesLocked() []router.PublishedAddress {
	addresses := make([]router.PublishedAddress, len(p.base))
	for i := range p.base {
		addresses[i] = p.base[i]
		addresses[i].Options = append([]router.MappingOption(nil), p.base[i].Options...)
		if mapping := p.active[i]; mapping != nil {
			addresses[i].Options = append(addresses[i].Options,
				router.MappingOption{Key: "host", Value: mapping.public.String()},
				router.MappingOption{Key: "port", Value: strconv.Itoa(int(mapping.externalPort))},
			)
		}
	}
	return addresses
...
var _ router.SocketAddressPublisher = (*natMappingPublisher)(nil)
var _ router.AddressPublisherCloser = (*natMappingPublisher)(nil)
