...
type fakeNATPMPClient struct {
	...
	requests         []natpmp.MappingRequest
	unmapCalls       int
...
func (f *fakeNATPMPClient) PublicAddress(context.Context) (natpmp.PublicAddress, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.events != nil {
	...
	return natpmp.PublicAddress{Address: f.public}, f.publicErr
}

func (f *fakeNATPMPClient) Map(_ context.Context, request natpmp.MappingRequest) (natpmp.Mapping, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.events != nil {
...
func (f *fakeNATPMPClient) Unmap(context.Context, natpmp.Mapping) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.unmapCalls++
	return nil
...
type fakeUPnPClient struct {
	events            *[]string
	discoverErr       error
	responses         []upnp.DiscoveryResponse
...
func (f *fakeUPnPClient) Discover(context.Context) ([]upnp.DiscoveryResponse, error) {
	f.discoverCalls++
	if f.events != nil {
...
func (f *fakeUPnPClient) Describe(_ context.Context, location *url.URL) (upnp.Gateway, error) {
	f.describedLocation = location
	return f.gateway, nil
}
func (f *fakeUPnPClient) ExternalAddress(context.Context, upnp.Gateway) (netip.Addr, error) {
	return f.public, nil
}
func (f *fakeUPnPClient) AddPortMapping(_ context.Context, _ upnp.Gateway, mapping upnp.PortMapping) error {
	f.mapping = mapping
	f.addCalls++
	return nil
}
func (f *fakeUPnPClient) DeletePortMapping(context.Context, upnp.Gateway, string, uint16, string) error {
	f.deleteCalls++
	return nil
}
...
func (wildcardSockets) ListenStream(context.Context, router.Endpoint) (net.Listener, error) {
	return net.Listen("tcp4", "0.0.0.0:0")
}
func (wildcardSockets) DialStream(ctx context.Context, endpoint router.Endpoint) (net.Conn, error) {
	return (&net.Dialer{}).DialContext(ctx, endpoint.Network, endpoint.Address)
}
func (wildcardSockets) ListenUDP(context.Context, router.Endpoint) (*net.UDPConn, error) {
...
func TestDaemonRemainsRunningWhenInitialMappingFailsAndRetries(t *testing.T) {
	...
	cfg.NTCP2 = config.Transport{Enabled: true, Bind: config.Endpoint{Host: "0.0.0.0"}, MaxSessions: 4}
...
func TestAutomaticMappingUsesNATPMPFirstAndObservedLease(t *testing.T) {
	...
	base := staticAddressPublisher{{Transport: "NTCP2", Options: []router.MappingOption{{Key: "v", Value: "2"}}}}
	...
	client.mu.Unlock()
	if request.ExternalPort != 0 || request.InternalPort == 0 || request.Lifetime != 2*time.Minute || request.Protocol != natpmp.TCP {
...
func TestAutomaticMappingFallsBackToUPnP(t *testing.T) {
	...
	upnpClient := &fakeUPnPClient{
		events: &events, responses: []upnp.DiscoveryResponse{{Location: location}},
		gateway: upnp.Gateway{ControlURL: control, ServiceType: "urn:schemas-upnp-org:service:WANIPConnection:2"},
...
func TestExplicitUPnPEndpointBypassesSSDPDiscovery(t *testing.T) {
	...
	client := &fakeUPnPClient{gateway: upnp.Gateway{ControlURL: control, ServiceType: "urn:schemas-upnp-org:service:WANIPConnection:2"}}
...
func TestMappingChangeRepublishesRouterInfo(t *testing.T) {
	local := newNATMappingTestLocal(t)
	base := staticAddressPublisher{{Transport: "NTCP2", Options: []router.MappingOption{{Key: "v", Value: "2"}}}}
	if err := local.ReplaceAddresses([]router.PublishedAddress(base)); err != nil {
		t.Fatal(err)
	}
	local.SetReachability(router.ReachabilityFirewalled)
...
func snapshotTransportOptions(t *testing.T, info netdb.RouterInfo, transport string) map[string]string {
	t.Helper()
...
func newNATMappingTestLocal(t *testing.T) *router.LocalRouterInfo {
	t.Helper()
...
func addressOptions(address router.PublishedAddress) map[string]string {
