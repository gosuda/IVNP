...
func (s *recordingSockets) ListenStream(context.Context, router.Endpoint) (net.Listener, error) {
	s.calls++
	return nil, errors.New("unexpected socket")
}
func (s *recordingSockets) DialStream(context.Context, router.Endpoint) (net.Conn, error) {
	s.calls++
	return nil, errors.New("unexpected socket")
}
func (s *recordingSockets) ListenUDP(context.Context, router.Endpoint) (*net.UDPConn, error) {
	s.calls++
	return nil, errors.New("unexpected socket")
}
...
func (s *blockingRequestSender) Send(ctx context.Context, _ netdb.RouterRef, _ i2np.Message) error {
...
func (s *requestDirectCapture) Send(_ context.Context, target ivnp.Hash, _ i2np.Message) error {
	s.calls++
	s.target = target
	return nil
}
...
func (s *requestTunnelCapture) SendBlock(_ context.Context, id uint32, block tunnel.Block) error {
	s.calls++
	s.id = id
	s.block = block
...
func (loopbackSockets) ListenStream(context.Context, router.Endpoint) (net.Listener, error) {
	return net.Listen("tcp", "127.0.0.1:0")
}
func (loopbackSockets) DialStream(ctx context.Context, endpoint router.Endpoint) (net.Conn, error) {
	return (&net.Dialer{}).DialContext(ctx, endpoint.Network, endpoint.Address)
}
func (loopbackSockets) ListenUDP(context.Context, router.Endpoint) (*net.UDPConn, error) {
...
func (s *defaultTransportSockets) ListenStream(context.Context, router.Endpoint) (net.Listener, error) {
	s.streams++
	return net.Listen("tcp", "127.0.0.1:0")
}
func (s *defaultTransportSockets) DialStream(ctx context.Context, endpoint router.Endpoint) (net.Conn, error) {
	return (&net.Dialer{}).DialContext(ctx, endpoint.Network, endpoint.Address)
}
func (s *defaultTransportSockets) ListenUDP(context.Context, router.Endpoint) (*net.UDPConn, error) {
	s.packets++
	return net.ListenUDP("udp4", &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1)})
}
...
type daemonMemoryTransport struct {
	...
	local    ivnp.Hash
	bindings router.TransportBindings
	done     chan struct{}
	once     sync.Once
	running  bool
}

func newDaemonMemoryNetwork(flood netdb.RouterInfo, now func() uint64) *daemonMemoryNetwork {
...
func (t *daemonMemoryTransport) Start(_ context.Context, bindings router.TransportBindings) error {
...
func (t *daemonMemoryTransport) Send(ctx context.Context, target ivnp.Hash, message i2np.Message) error {
...
func (t *daemonMemoryTransport) Status() router.TransportStatus {
	return router.TransportStatus{Running: t.running}
}

func (n *daemonMemoryNetwork) route(from, target ivnp.Hash, message i2np.Message) error {
	if target == n.flood {
		return n.handleFlood(from, message)
	}
	if message.Header.Type == i2np.ShortTunnelBuild {
		if _, err := i2np.ParseBuildRecords(i2np.ShortTunnelBuild, message.Payload); err != nil {
			return err
		}
	}
	n.mu.RLock()
...
func (n *daemonMemoryNetwork) handleFlood(from ivnp.Hash, message i2np.Message) error {
	...
	case i2np.DatabaseStore:
		...
		if store.ReplyToken != 0 {
			...
			status := i2np.Message{Header: i2np.Header{Type: i2np.DeliveryStatus, ID: n.messageID(), Expiration: n.now() + 60_000}, Payload: payload[:]}
	...
	case i2np.DatabaseLookup:
		...
		payload, err := netdb.MarshalDatabaseStore(lookup.Key, typeID, data, 0, ivnp.Hash{}, 0)
		if err != nil {
			return err
		}
		reply := i2np.Message{Header: i2np.Header{Type: i2np.DatabaseStore, ID: n.messageID(), Expiration: n.now() + 60_000}, Payload: payload}
...
func (n *daemonMemoryNetwork) reply(from, gateway ivnp.Hash, tunnelID uint32, message i2np.Message) error {
	if tunnelID == 0 {
	...
	gatewayMessage := i2np.Message{Header: i2np.Header{Type: i2np.TunnelGateway, ID: n.messageID(), Expiration: message.Header.Expiration}, Payload: payload}
	return n.route(from, gateway, gatewayMessage)
}

func daemonProductionFloodfill(t *testing.T, now uint64) netdb.RouterInfo {
	t.Helper()
...
func TestDefaultOperatingStartsOutboundNTCP2AndBoundSSU2(t *testing.T) {
	cfg, err := config.ParseOperating("", filepath.Join(t.TempDir(), "ivnp.conf"))
	if err != nil {
		t.Fatal(err)
	}
...
func daemonTestConfig(t *testing.T) config.Operating {
	t.Helper()
	base := t.TempDir()
	return config.Operating{
		StatePath: filepath.Join(base, "router.state"),
		KeyPath:   filepath.Join(base, "router.keys"),
		Network:   config.Network{ID: 2, IPv4: true},
		Router:    config.Router{Version: "0.0.0"},
		State:     config.State{MaxBytes: 1 << 20, MaxDestinations: 16, MaxNameBytes: 64},
		NetDB:     config.NetDB{BucketCapacity: 4},
		...
		NTCP2: config.Transport{Enabled: true, Bind: config.Endpoint{Host: "127.0.0.1", Port: 12345}, MaxSessions: 4},
	}
}
...
func TestNewDoesNotOpenSocketsAndReloadsState(t *testing.T) {
	...
	if _, err := New(cfg, Options{SocketRuntime: sockets}); !errors.Is(err, state.ErrStateLocked) {
...
func TestNewRejectsEnabledTunnelLifetimeOutsideWireLifetime(t *testing.T) {
	for _, lifetime := range []time.Duration{9 * time.Minute, 11 * time.Minute} {
		t.Run(lifetime.String(), func(t *testing.T) {
			...
			if _, err := New(cfg, Options{SocketRuntime: new(recordingSockets)}); !errors.Is(err, config.ErrInvalidOperating) {
...
func TestMuxRequestSenderUsesEstablishedOutboundTunnel(t *testing.T) {
	...
	payload, err := netdb.BuildDatabaseLookup(ivnp.Hash{3}, netdb.LeaseSetLookup, requestReplyRouteCapture{
	...
	replyKeys := garlic.NewReplyKeyRegistry(2)
	...
	message := i2np.Message{Header: i2np.Header{Type: i2np.DatabaseLookup, ID: 9, Expiration: 1_000}, Payload: payload}
	...
	if throughTunnel.block.Delivery != tunnel.DeliveryRouter || throughTunnel.block.Gateway != target || !throughTunnel.block.Last {
		t.Fatalf("tunnel block = %#v", throughTunnel.block)
	}
	decoded, used, err := i2np.Parse(throughTunnel.block.Data)
	if err != nil || used != len(throughTunnel.block.Data) || decoded.Header.Type != i2np.Garlic {
	...
	inner, err := eciesgarlic.OpenRouterMessage(make([]byte, len(decoded.Payload)), private.Bytes(), decoded.Payload[4:], 100)
...
func TestMuxLeaseSetSenderUsesOutboundTunnelAndSeedsBothRoutes(t *testing.T) {
	...
	payload, err := netdb.MarshalDatabaseStore(ivnp.Hash{4}, i2np.StoreLeaseSet2, []byte{1}, 5, replyGateway, 7)
	if err != nil {
		t.Fatal(err)
	}
	direct := new(requestDirectCapture)
	...
	message := i2np.Message{Header: i2np.Header{Type: i2np.DatabaseStore, ID: 9, Expiration: 1_000}, Payload: payload}
	if err = sender.Send(context.Background(), netdb.RouterRef{Hash: target}, message); err != nil {
		t.Fatal(err)
	}
	if direct.calls != 0 || throughTunnel.calls != 1 || throughTunnel.id != 11 {
	...
	store, err := i2np.ParseDatabaseStore(decoded.Payload)
...
func TestProxyRequiresTunnelAndPersistsDefaultDestination(t *testing.T) {
	...
	cfg.HTTPProxy.Address = config.Endpoint{Host: "127.0.0.1"}
...
func TestStaticNTCP2PublisherRequiresExplicitAdvertisement(t *testing.T) {
	...
	cfg.NTCP2.Advertised = config.Endpoint{Host: "127.0.0.1", Port: 12345}
	options = publishedOptions()
...
func TestStartServesAllLocalListenersAndCloses(t *testing.T) {
	cfg := daemonTestConfig(t)
	cfg.Tunnel.Enabled = true
	cfg.Control = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, BearerToken: "test-token", MaxConnections: 4}
	cfg.HTTPProxy = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 4}
	cfg.SOCKS5 = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 4}
	cfg.Metrics = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 4}
...
func TestDestinationAddressPoliciesPersistAndWireRemoteELS(t *testing.T) {
	...
	policies := []state.RemoteELSAuthorization{
		{Identity: append([]byte(nil), noneIdentity.Bytes()...), Secret: []byte("none")},
		{Identity: append([]byte(nil), dhIdentity.Bytes()...), Secret: []byte("dh"), Kind: state.RemoteELSAuthorizationDH, DHPrivate: private, DHPublic: public},
		{Identity: append([]byte(nil), pskIdentity.Bytes()...), Secret: []byte("psk"), Kind: state.RemoteELSAuthorizationPSK, PSK: pskKey},
	}
	...
	if err = d.UpdateDestinationAddressPolicies("default", invalid); !errors.Is(err, router.ErrDataPlaneConfig) {
	...
	if got := reopened.bundle.DestinationAddressPolicies["default"]; len(got) != len(policies) || got[1].Kind != state.RemoteELSAuthorizationDH || got[2].Kind != state.RemoteELSAuthorizationPSK {
...
func TestDaemonNewProductionGraphPublicAndEncryptedDestinations(t *testing.T) {
	...
	for _, admission := range []struct {
		...
		{publicDaemon.database, []netdb.RouterInfo{encryptedInfo, transitInfo}},
		{encryptedDaemon.database, []netdb.RouterInfo{publicInfo, transitInfo}},
		{transitDaemon.database, []netdb.RouterInfo{publicInfo, encryptedInfo}},
	} {
	...
	network.mu.RLock()
	stores := append([]i2np.DatabaseStoreMessage(nil), network.stores...)
	network.mu.RUnlock()
	...
	if err = publicDaemon.UpdateDestinationAddressPolicies("public", []state.RemoteELSAuthorization{{
		Identity: append([]byte(nil), encryptedIdentity.Bytes()...), Secret: []byte("production-secret"), Kind: state.RemoteELSAuthorizationNone,
	}}); err != nil {
		t.Fatal(err)
	}
	...
	messageRoute := clientapi.DestinationRoute{Protocol: 17, ToPort: 4444}
	...
	if err = publicSession.SendMessage(context.Background(), streamtunnel.Delivery{
	...
	selfRoute := clientapi.DestinationRoute{Protocol: 17, ToPort: 5555}
	...
	exercise := func(source, target *router.DestinationSession, port string, request, response []byte) {
...
func TestDestroyRemovesExactReleasedDestinationRuntime(t *testing.T) {
	...
	if _, lookupErr := removed.requests.LookupRouterInfo(context.Background(), ivnp.Hash{99}); !errors.Is(lookupErr, netdb.ErrRequestManagerClosed) {
	...
	if senderErr := removed.sender.UpdateRemoteELS(nil); !errors.Is(senderErr, router.ErrDataPlaneConfig) {
...
func TestWaitDoesNotRaceStartRegistration(t *testing.T) {
	cfg := daemonTestConfig(t)
	cfg.Metrics = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 1}
...
func TestOptionsListenOwnsEveryClientListener(t *testing.T) {
	cfg := daemonTestConfig(t)
	cfg.Tunnel.Enabled = true
	cfg.Control = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, BearerToken: "control-token", MaxConnections: 1}
	cfg.HTTPProxy = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 1}
	cfg.SOCKS5 = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 1}
	cfg.Metrics = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, MaxConnections: 1}
	var calls int
	d, err := New(cfg, Options{
		SocketRuntime: loopbackSockets{},
...
func TestOptionsListenerInjectsSAMPacketListener(t *testing.T) {
	cfg := daemonTestConfig(t)
	cfg.Tunnel.Enabled = true
	cfg.SAM = config.Listener{
		Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, UDPAddress: config.Endpoint{Host: "127.0.0.1"},
...
func TestMetricsBearerAuthentication(t *testing.T) {
	cfg := daemonTestConfig(t)
	cfg.Metrics = config.Listener{Enabled: true, Address: config.Endpoint{Host: "127.0.0.1"}, BearerToken: "metrics-token", MaxConnections: 1}
	d, err := New(cfg, Options{SocketRuntime: loopbackSockets{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteELSContextFailureAndReleaseCleanup(t *testing.T) {
	...
	invalid := []state.RemoteELSAuthorization{
		{Identity: append([]byte(nil), identity.Bytes()...), Secret: []byte("first"), Kind: state.RemoteELSAuthorizationNone},
		{Identity: append([]byte(nil), identity.Bytes()...), Secret: []byte("second"), Kind: state.RemoteELSAuthorizationKind(99)},
	}
	if contexts, err := remoteELSContexts(invalid); !errors.Is(err, router.ErrDataPlaneConfig) || contexts != nil {
		t.Fatalf("remoteELSContexts partial failure = %#v, %v", contexts, err)
	}
	secret := []byte("retained")
	contexts := map[ivnp.Hash]router.RemoteELSContext{
