...
func (s *Server) handleSend(connection *serverConnection, cmd command) error {
	...
	if cmd.verb == "DATAGRAM" {
		...
		payload = framed[:n]
		protocol = datagram.ProtocolDatagram1
	}
	if cmd.verb == "RAW" {
	...
	err = session.endpoint.SendMessage(session.ctx, streamtunnel.Delivery{From: session.endpoint.Hash(), To: hash, FromPort: uint16(fromPort), ToPort: uint16(toPort), Protocol: protocol, Payload: payload})
	if framed != nil {
		clear(framed)
	}
	if err != nil {
...
func (s *samSession) receiveLoop(subscription clientapi.MessageSubscription) {
	...
	for {
		...
		func() {
			...
			switch s.style {
			case styleDatagram:
				packet, parseErr := datagram.ParsePacket(datagram.ProtocolDatagram1, delivery.Payload)
				if parseErr != nil {
					return
				}
