...
func (s *Server) sendUDPPacket(packet udpPacket) {
	...
	_ = session.endpoint.SendMessage(session.ctx, streamtunnel.Delivery{From: session.endpoint.Hash(), To: hash, FromPort: packet.fromPort, ToPort: packet.toPort, Protocol: protocol, Payload: payload})
	clear(framed)
}
