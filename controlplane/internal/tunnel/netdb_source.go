...
func NewNetDBOutboundBuildSource(config NetDBOutboundBuildSourceConfig) (*NetDBOutboundBuildSource, error) {
	newNetDBOutboundBuildSourceRejected := config.Table == nil || config.Profiles == nil || (config.ReplyRouter == (foundation.Hash{})) != (config.ReplyTunnelID == 0) || config.Hops < 1 || config.Hops > foundation.I2NPMaxVariableBuildRecords || config.Lifetime == 0 || config.CircuitID == nil
...
func sortCandidatesCanonical(candidates []controlplanenetdb.RouterRef) {
	slices.SortFunc(candidates, func(left, right controlplanenetdb.RouterRef) int {
...
func NewNetDBInboundBuildSource(config NetDBInboundBuildSourceConfig) (*NetDBInboundBuildSource, error) {
	newNetDBInboundBuildSourceRejected := config.Table == nil || config.Profiles == nil || config.LocalRouter == (foundation.Hash{}) || config.Hops < 1 || config.Hops >= foundation.I2NPMaxVariableBuildRecords || config.Lifetime == 0 || config.CircuitID == nil
...
func NewNetDBBuildStaticKeyLookup(table *controlplanenetdb.Table) BuildStaticKeyLookup {
	if table == nil {
		return nil
	}
...
func x25519StaticKey(info foundation.NetworkDatabaseRouterInfo) ([32]byte, bool) {
...
func tunnelPeerCapabilitiesAllowed(info foundation.NetworkDatabaseRouterInfo, exploratory, allowRestricted bool) (tunnelPeerCapabilities, bool) {
	return tunnelPeerCapabilitiesAllowedWithDecisions(info, exploratory, allowRestricted, allowRestricted, allowRestricted)
}

func tunnelPeerCapabilitiesAllowedWithDecisions(info foundation.NetworkDatabaseRouterInfo, exploratory, allowCongested, allowFloodfill, allowUnreachable bool) (tunnelPeerCapabilities, bool) {
...
func tunnelPeerMetadata(info foundation.NetworkDatabaseRouterInfo) (version, caps []byte, ok bool) {
...
func tunnelPeerTransportMask(info foundation.NetworkDatabaseRouterInfo) tunnelTransportMask {
...
func routerMetadata(info foundation.NetworkDatabaseRouterInfo) (string, [][4]byte, [][16]byte, []uint16) {
	var family string
	options := info.Options.Iterator()
	for {
