...
func (m *BuildManager) handleVariableTransit(ctx context.Context, message foundation.I2NPMessage) (result error) {
	if m.local == (foundation.Hash{}) {
		return ErrBuildConfig
	}
	records, err := foundation.I2NPParseBuildRecords(foundation.I2NPVariableTunnelBuild, message.Payload)
	if err != nil {
		return err
	}
	now := m.now()
	...
	if accept {
		installed, err = m.installVariableTransitCircuit(request, now)
		accept = err == nil
	}
	if accept {
	...
	if request.Endpoint {
		...
		if err = m.sendVariableReply(ctx, request, message.Payload, now); err != nil {
			return err
		}
		if !accept {
			return ErrBuildRejected
	...
	forward := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPVariableTunnelBuild, ID: request.NextMessageID, Expiration: saturatingDeadline(now, nextHopSendTimeout)}, Payload: message.Payload}
...
func (m *BuildManager) sendVariableReply(ctx context.Context, request VariableBuildRequest, records []byte, now uint64) error {
	reply := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPVariableTunnelBuildReply, ID: request.NextMessageID, Expiration: saturatingDeadline(now, nextHopSendTimeout)}, Payload: records}
	...
	gateway := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPTunnelGateway, ID: request.NextMessageID, Expiration: saturatingDeadline(now, nextHopSendTimeout)}, Payload: payload}
	return m.sender.Send(ctx, request.NextRouter, gateway)
}
...
func (m *BuildManager) HandleVariableReply(message foundation.I2NPMessage) error {
	m.lifecycleMu.RLock()
	defer m.lifecycleMu.RUnlock()
	if m.isReleased() {
	...
	for hop := range pending.keys {
		...
		if transforms[hop], err = dataplane.TunnelNewLayerEncryptor(key.LayerKey[:], key.IVKey[:]); err != nil {
			return err
		}
	}
	...
	circuit := dataplane.TunnelOutboundCircuit{ID: pending.build.CircuitID, Owner: owner, FirstHop: pending.build.Hops[0].Router, NextTunnelID: pending.build.Hops[0].ReceiveTunnelID, Transforms: transforms, ExpiresAt: pending.build.ExpiresAt}
	entry := Entry{ID: circuit.ID, Owner: owner, Direction: Outbound, Expires: circuit.ExpiresAt}
...
func (m *BuildManager) installVariableTransitCircuit(request VariableBuildRequest, now uint64) (dataplane.TunnelCircuitToken, error) {
	decryptor, err := dataplane.TunnelNewLayerDecryptor(request.LayerKey[:], request.IVKey[:])
	if err != nil {
		return dataplane.TunnelCircuitToken{}, err
	}
	circuit := dataplane.TunnelInboundCircuit{ID: request.ReceiveTunnelID, Transforms: []dataplane.TunnelLayerCipher{decryptor}, ExpiresAt: now + 660_000}
	if request.Endpoint {
		circuit.Endpoint, circuit.Local = dataplane.TunnelNewEndpoint(128, 0), m.localDelivery
	} else {
