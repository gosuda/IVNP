...
func (s *rotationFailingSender) Send(context.Context, foundation.Hash, foundation.I2NPMessage) error {
	s.calls++
	return s.err
}
...
func TestRotatorRenewalPreservesOldCircuitUntilExpiry(t *testing.T) {
	...
	if _, err := runtime.RegisterOutbound(dataplane.TunnelOutboundCircuit{ID: old.ID, NextTunnelID: 11, ExpiresAt: old.Expires}); err != nil {
		t.Fatal(err)
	}
	...
	if err = manager.HandleReply(foundation.I2NPMessage{
		Header:  foundation.I2NPHeader{Type: foundation.I2NPOutboundTunnelBuildReply, ID: pending.replyID},
		Payload: payload,
	}); err != nil {
		t.Fatal(err)
	}
...
func TestRotatorExpiresPoolRuntimeAndPendingState(t *testing.T) {
	...
	if _, err := runtime.RegisterOutbound(dataplane.TunnelOutboundCircuit{ID: 1, NextTunnelID: 2, ExpiresAt: now + 1}); err != nil {
		t.Fatal(err)
	}
	builder, err := NewBuildManager(BuildManagerConfig{
		Runtime: runtime, Pool: pool, Sender: sender, ReplyKeys: newBuildReplyRegistry(), Now: func() uint64 { return now }, Random: new(buildCounterReader),
	...
	if err = runtime.SendBlock(context.Background(), 1, dataplane.TunnelBlock{Delivery: dataplane.TunnelDeliveryLocal, Last: true, Data: []byte{1}}); err != dataplane.TunnelErrCircuitNotFound {
		t.Fatalf("expired runtime circuit error=%v, want %v", err, dataplane.TunnelErrCircuitNotFound)
	}
}
...
func TestRotatorReturnsSourceAndBuildFailuresWithoutRetainingPending(t *testing.T) {
	...
	source := &rotationSource{err: errRotationSource}
	builder, err := NewBuildManager(BuildManagerConfig{Runtime: runtime, Pool: pool, Sender: buildDiscardSender{}, ReplyKeys: newBuildReplyRegistry(), Now: func() uint64 { return now }})
	if err != nil {
		t.Fatal(err)
	}
	rotator, err := NewRotator(RotatorConfig{Pool: pool, Runtime: runtime, Builder: builder, Source: source, Now: func() uint64 { return now }, Target: 1})
