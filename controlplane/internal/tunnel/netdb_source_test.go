...
func verifiedX25519Router(t *testing.T, marker byte) foundation.NetworkDatabaseRouterInfo {
	t.Helper()
	return verifiedX25519RouterTransport(t, marker, "NTCP2", "192.0.2.1", "", "")
}

func verifiedX25519RouterTransport(t *testing.T, marker byte, style, host, addressCaps, routerCaps string) foundation.NetworkDatabaseRouterInfo {
	t.Helper()
	return verifiedX25519RouterTransportVersion(t, marker, style, host, addressCaps, routerCaps, "0.9.70")
}

func verifiedX25519RouterTransportVersion(t *testing.T, marker byte, style, host, addressCaps, routerCaps, version string) foundation.NetworkDatabaseRouterInfo {
	t.Helper()
	...
	info, err := foundation.NetworkDatabaseParseRouterInfo(append(unsigned, signature...))
	if err != nil {
		t.Fatal(err)
	}
	return info
...
func TestNetDBBuildSourcesUseFullProfilePopulation(t *testing.T) {
	...
	all := table.ClosestInto(make([]controlplanenetdb.RouterRef, table.Len()), target)
	outside := all[2].Hash
	profiles := NewPeerProfiles(PeerProfilesConfig{Window: 4})
