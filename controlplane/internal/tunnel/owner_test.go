...
func TestRuntimeRetainsCreatorOwnerOnInstalledCircuit(t *testing.T) {
	...
	owner[0] = 9
	runtime := dataplane.TunnelNewRuntime(dataplane.TunnelRuntimeConfig{Now: func() uint64 { return 1 }})
	if _, err := runtime.RegisterOutbound(dataplane.TunnelOutboundCircuit{ID: 7, Owner: owner, NextTunnelID: 8, ExpiresAt: 100}); err != nil {
		t.Fatal(err)
	}
	...
	installed, _ := runtime.InspectCircuit(7)
