...
func (s *fakeSockets) ListenStream(context.Context, dataplane.RouterEndpoint) (net.Listener, error) {
...
func (l *fakeLocalInfo) Snapshot() foundation.NetworkDatabaseRouterInfo {
...
func (t *fakeTransport) Start(_ context.Context, bindings dataplane.RouterTransportBindings) error {
	t.bindings = bindings
...
func (t *fakeTransport) Send(context.Context, foundation.Hash, foundation.I2NPMessage) error {
	return nil
}
func (t *fakeTransport) Status() dataplane.RouterTransportStatus {
	return dataplane.RouterTransportStatus{}
}
...
func newRuntimeForTest(t *testing.T, sockets dataplane.RouterSocketRuntime, transport dataplane.RouterTransportManager, log *eventLog) *Router {
	t.Helper()
	database := controlplanenetdb.NewDatabase(foundation.Hash{}, 1)
	router, err := New(Config{
		NTCP2: dataplane.RouterEndpoint{Network: "tcp", Address: "127.0.0.1:0"},
		SSU2:  dataplane.RouterEndpoint{Network: "udp", Address: "127.0.0.1:0"},
	}, Dependencies{
		Database: database, LocalInfo: &fakeLocalInfo{log: log}, Transport: transport,
		Sockets: sockets, Addresses: &fakeAddresses{log: log}, Clock: dataplane.RouterWallClock{},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func newRuntimeWithStreamBackendForTest(t *testing.T, backend StreamBackend) *Router {
	...
	router, err := New(Config{}, Dependencies{
		...
		Sockets: &fakeSockets{log: log}, Addresses: &fakeAddresses{log: log}, Clock: dataplane.RouterWallClock{},
...
func newReseedRuntimeForTest(t *testing.T, cfg Config, log *eventLog, reseed ReseedRunner, clock dataplane.RouterClock) (*Router, *controlplanenetdb.Database, *fakeTransport) {
	t.Helper()
	database := controlplanenetdb.NewDatabase(foundation.Hash{}, 1)
...
func TestRouterRollsBackSocketOnStartupFailure(t *testing.T) {
	...
	if err := router.Start(context.Background()); !errors.Is(err, dataplane.RouterErrStarted) {
...
func TestRouterReseedsWithFiftyTransportStaleRetainedPeers(t *testing.T) {
	...
	database := controlplanenetdb.NewDatabase(foundation.Hash{}, 64)
	runtime.deps.Database = database

	published := uint64(now.UnixMilli()) - controlplanenetdb.RouterInfoMaxAgeMillis - 1
	for range reseedBootstrapMinimum {
		...
		database.Routers().StoreVerified(foundation.NetworkDatabaseRouterInfo{Identity: identity, Published: published}, false, uint64(now.UnixMilli()))
	}
...
func TestRouterControlWorkDoesNotBlockTransportDataAndJoinsOnClose(t *testing.T) {
	log := new(eventLog)
	transport := newFakeTransport(log)
	started := make(chan dataplane.RouterI2NPSource, 1)
	stopped := make(chan struct{})
	delivered := make(chan uint32, 1)
	service := dataplane.RouterNewService(dataplane.RouterSinks{TunnelData: func(message foundation.I2NPMessage) error {
		delivered <- message.Header.ID
		return nil
	}})
	router, err := New(Config{}, Dependencies{
		Database: controlplanenetdb.NewDatabase(foundation.Hash{}, 1), Service: service,
		LocalInfo: &fakeLocalInfo{log: log}, Transport: transport,
		Sockets: &fakeSockets{log: log}, Addresses: &fakeAddresses{log: log}, Clock: dataplane.RouterWallClock{},
		ControlSinks: ControlSinks{TunnelBuild: func(ctx context.Context, source dataplane.RouterI2NPSource, _ foundation.I2NPBuildRecords, _ foundation.I2NPMessage) error {
			started <- source
			<-ctx.Done()
			close(stopped)
			return ctx.Err()
	...
	if err = transport.bindings.HandleI2NPContext(t.Context(), peer, build, 1_000, false); err != nil {
		t.Fatal(err)
	}
	select {
	case source := <-started:
		if source != (dataplane.RouterI2NPSource{Peer: peer, Direct: true}) {
			t.Fatalf("control source = %#v", source)
		}
	...
	payload[3] = 1
	data := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPTunnelData, ID: 2, Expiration: 1_000}, Payload: payload}
	if err = transport.bindings.HandleI2NPContext(t.Context(), peer, data, 1_000, false); err != nil {
		t.Fatal(err)
	}
	select {
	case id := <-delivered:
		if id != 2 {
			t.Fatalf("delivered data ID = %d", id)
		}
	default:
		t.Fatal("data delivery waited for blocked control handler")
	}
	if err = router.Close(); err != nil {
		t.Fatal(err)
	}
	...
	case <-stopped:
	default:
		t.Fatal("router shutdown returned before control handler exited")
	}
	if err = router.Wait(); err != nil {
		t.Fatal(err)
	}
...
type closingRequestHandler struct {
	started chan struct{}
	closed  chan struct{}
	once    sync.Once
}

func (h *closingRequestHandler) HandleDatabaseSearchReply(context.Context, foundation.I2NPDatabaseSearchReplyMessage) {
	close(h.started)
	<-h.closed
}

func (*closingRequestHandler) ExpectsDatabaseStore(foundation.I2NPDatabaseStoreMessage) bool {
	return false
}
func (*closingRequestHandler) HandleDatabaseStore(context.Context, foundation.I2NPDatabaseStoreMessage) {
}
func (h *closingRequestHandler) Close() error {
	h.once.Do(func() { close(h.closed) })
	return nil
}

func TestRouterCancelsRequestOwnerBeforeJoiningControlIngress(t *testing.T) {
	log := new(eventLog)
	transport := newFakeTransport(log)
	requests := &closingRequestHandler{started: make(chan struct{}), closed: make(chan struct{})}
	router, err := New(Config{}, Dependencies{
		Database: controlplanenetdb.NewDatabase(foundation.Hash{}, 1), LocalInfo: &fakeLocalInfo{log: log},
		Transport: transport, Sockets: &fakeSockets{log: log}, Addresses: &fakeAddresses{log: log},
		Clock: dataplane.RouterWallClock{}, RequestHandler: requests,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	message := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPDatabaseSearchReply, ID: 1, Expiration: 1_000}, Payload: make([]byte, 65)}
	if err = transport.bindings.HandleI2NP(message, 1_000, false); err != nil {
		t.Fatal(err)
	}
	select {
	case <-requests.started:
	case <-time.After(5 * time.Second):
		t.Fatal("request continuation did not start")
	}
	...
	case <-time.After(5 * time.Second):
		t.Fatal("control join waited for an uncanceled request owner")
	}
}

func TestRouterPreservesEstablishedTunnelSender(t *testing.T) {
	const now = uint64(1_000)
	unavailable := errors.New("established tunnel transport unavailable")
	wire := &controlPlaneTunnelSender{handle: func(context.Context, foundation.I2NPMessage) error { return unavailable }}
	tunnels := dataplane.TunnelNewRuntime(dataplane.TunnelRuntimeConfig{Sender: wire, Now: func() uint64 { return now }})
	token, err := tunnels.RegisterOutbound(dataplane.TunnelOutboundCircuit{ID: 1, FirstHop: foundation.Hash{7}, NextTunnelID: 2, ExpiresAt: now + 1_000})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { tunnels.RemoveCircuit(token) })
	log := new(eventLog)
	router, err := New(Config{}, Dependencies{
		Database: controlplanenetdb.NewDatabase(foundation.Hash{}, 1), LocalInfo: &fakeLocalInfo{log: log},
		Transport: newFakeTransport(log), Sockets: &fakeSockets{log: log}, Addresses: &fakeAddresses{log: log},
		Clock: dataplane.RouterWallClock{}, Tunnels: tunnels,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	message := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPDeliveryStatus, ID: 3, Expiration: now}, Payload: make([]byte, 12)}
	if err = tunnels.HandleGateway(1, message); !errors.Is(err, unavailable) {
		t.Fatalf("configured established sender was replaced: %v", err)
	}
}
