...
func TestLocalRouterInfoPublishesRouterLifecycleState(t *testing.T) {
	...
	database := controlplanenetdb.NewDatabase(foundation.Hash{}, 4)
...
func TestLocalRouterInfoAdvertisesConfiguredFloodfillWithReachabilityState(t *testing.T) {
	...
	if foundation.NetworkDatabaseIsFloodfill(owner.Snapshot()) {
	...
	if info := owner.Snapshot(); !foundation.NetworkDatabaseIsFloodfill(info) || mappingValue(t, info.Options, "caps") != "KfU" {
