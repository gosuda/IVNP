...
type LocalRouterInfo struct {
	info     *controlplanenetdb.LocalRouterInfo
	database *controlplanenetdb.Database
	clock    dataplane.RouterClock
	metrics  *observability.Registry
...
func NewLocalRouterInfo(config LocalRouterInfoConfig) (*LocalRouterInfo, error) {
	...
	info, err := controlplanenetdb.NewLocalRouterInfo(controlplanenetdb.LocalRouterInfoConfig{
		Local:    config.Local,
		Contacts: contacts,
	})
	if err != nil {
...
func (l *LocalRouterInfo) Snapshot() foundation.NetworkDatabaseRouterInfo {
...
func (l *LocalRouterInfo) UpdateSSU2Introducers(ctx context.Context, leases []dataplane.RouterSSU2Introducer) error {
	if ctx ==
		nil {
		ctx = context.Background()
	}
	...
	owned := append([]dataplane.RouterSSU2Introducer(nil), leases...)
	if len(owned) > 3 {
...
func (l *LocalRouterInfo) contactsLocked(addresses []PublishedAddress, reachability Reachability) (controlplanenetdb.RouterInfoContacts, error) {
	...
	contacts := controlplanenetdb.RouterInfoContacts{
		Addresses: make([]controlplanenetdb.LocalRouterAddress, len(addresses)),
	...
	for index, address := range addresses {
		...
		if len(address.Transport) == 0 || len(address.Transport) > 255 {
			return controlplanenetdb.RouterInfoContacts{}, ErrLocalRouterInfoOptions
		}
		contacts.Addresses[index] = controlplanenetdb.LocalRouterAddress{
			Cost:           address.Cost,
