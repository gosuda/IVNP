...
type TunnelBuildReplyHandler interface {
	HandleInboundReply(foundation.I2NPMessage) error
	HandleReply(foundation.I2NPMessage) error
}

// NetDBRequestHandler advances all request domains which may own an admitted
// lookup result.
type NetDBRequestHandler interface {
	HandleDatabaseSearchReply(context.Context, foundation.I2NPDatabaseSearchReplyMessage)
	ExpectsDatabaseStore(foundation.I2NPDatabaseStoreMessage) bool
	HandleDatabaseStore(context.Context, foundation.I2NPDatabaseStoreMessage)
	Close() error
}
...
type Dependencies struct {
	...
	Sockets      dataplane.RouterSocketRuntime
	...
	BuildManager  *controlplanetunnel.BuildManager
	...
	LookupResponder *controlplanenetdb.LookupResponder
	// StoreFlooder owns bounded DatabaseStore propagation for floodfill nodes.
	StoreFlooder *controlplanenetdb.StoreFlooder
	...
	RouterDelivery func(foundation.Hash, foundation.I2NPMessage) error
	TunnelDelivery func(foundation.Hash, uint32, foundation.I2NPMessage) error
...
type Status struct {
	State     State
	Error     error
	Transport dataplane.RouterTransportStatus
}
...
func New(cfg Config, deps Dependencies) (*Router, error) {
	...
	if deps.Service == nil {
		deps.Service = dataplane.RouterNewService(dataplane.RouterSinks{})
	}
	controlSinks := deps.ControlSinks
	...
	if deps.BuildManager != nil {
		handleReply := func(message foundation.I2NPMessage) error {
			err := deps.BuildManager.HandleReply(message)
			if !errors.Is(err, controlplanetunnel.ErrBuildPending) {
				return err
			}
			if deps.ClientBuildReplies != nil {
				return deps.ClientBuildReplies.HandleReply(message)
			}
			return controlplanetunnel.ErrBuildPending
		}
		controlSinks.TunnelBuild = func(ctx context.Context, source dataplane.RouterI2NPSource, _ foundation.I2NPBuildRecords, message foundation.I2NPMessage) error {
			if message.Header.Type == foundation.I2NPVariableTunnelBuildReply {
				return handleReply(message)
			}
			if message.Header.Type == foundation.I2NPShortTunnelBuild && deps.ClientBuildReplies != nil {
				err := deps.ClientBuildReplies.HandleInboundReply(message)
				if !errors.Is(err, controlplanetunnel.ErrBuildPending) {
					return err
				}
			}
			return deps.BuildManager.HandleBuildContext(ctx, controlplanetunnel.BuildSource{Router: source.Peer, Direct: source.Direct}, message)
		}
		controlSinks.OutboundTunnelBuildReply = handleReply
	}
	if deps.RequestHandler != nil {
		controlSinks.DatabaseSearchReply = func(ctx context.Context, reply foundation.I2NPDatabaseSearchReplyMessage) error {
			deps.RequestHandler.HandleDatabaseSearchReply(ctx, reply)
			return nil
		}
		controlSinks.DatabaseStoreCompleted = deps.RequestHandler.HandleDatabaseStore
		controlSinks.DatabaseStoreExpected = deps.RequestHandler.ExpectsDatabaseStore
	}
	if deps.LookupResponder != nil {
		controlSinks.DatabaseLookup = deps.LookupResponder.Enqueue
	}
	if deps.StoreFlooder != nil {
		controlSinks.DatabaseStoreFlood = func(source dataplane.RouterI2NPSource, store foundation.I2NPDatabaseStoreMessage) error {
			return deps.StoreFlooder.Enqueue(source.Peer, store)
		}
	}
	if deps.DatabaseStoreReply != nil {
		controlSinks.DatabaseStoreReply = deps.DatabaseStoreReply
	}
	if deps.DeliveryStatusMux != nil {
		controlSinks.DeliveryStatus = deps.DeliveryStatusMux.Sink
	}
	if deps.TunnelTest != nil {
		controlSinks.TunnelTest = func(status foundation.I2NPDeliveryStatusMessage) error {
	...
	if deps.GarlicReceiver != nil {
		...
		if deps.Destinations != nil {
			deps.Service.SetDestinationSink(func(from, to foundation.Hash, message foundation.I2NPMessage) error {
				return deps.GarlicReceiver.HandleDestinationData(from, to, message, deps.Destinations)
			})
		}
	}
	limits := cfg.ControlQueue
	if limits == (dataplane.RouterControlQueueLimits{}) {
		limits = dataplane.RouterDefaultControlQueueLimits()
	}
	control, err := NewControlDispatcher(deps.Database, controlSinks, ControlDispatcherConfig{QueueLimits: limits, HandlerTimeout: cfg.ControlHandlerTimeout})
	if err != nil {
		return nil, err
	}
	deps.Service.SetControlIngress(control)
	return &Router{cfg: cfg, deps: deps, control: control, state: StateNew, done: make(chan struct{})}, nil
}
...
// WaitControl orders route preparation after already-admitted control work.
func (r *Router) WaitControl(ctx context.Context) error {
	return r.control.Barrier(ctx)
}
...
func (r *Router) stop(cause error, fatal bool) {
	r.closeOnce.Do(func() {
		...
		if r.deps.RequestHandler != nil {
			r.recordCloseError(r.deps.RequestHandler.Close())
		}
		r.recordCloseError(r.control.Close())
		if r.deps.LookupResponder != nil {
