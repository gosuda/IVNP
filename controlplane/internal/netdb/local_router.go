...
func (r *LocalRouterInfo) Publish(published uint64) (foundation.NetworkDatabaseRouterInfo, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *LocalRouterInfo) Snapshot() (foundation.NetworkDatabaseRouterInfo, bool) {
	r.mu.RLock()
	if len(r.raw) == 0 {
		r.mu.RUnlock()
...
func cloneRouterInfoContacts(contacts RouterInfoContacts) (RouterInfoContacts, error) {
	if len(contacts.Addresses) > foundation.NetworkDatabaseMaxRouterAddresses || len(contacts.Peers) > foundation.NetworkDatabaseMaxRouterPeers {
		return RouterInfoContacts{}, foundation.NetworkDatabaseErrTooManyItems
	}
	...
	for index, address := range contacts.Addresses {
		if len(address.TransportStyle) == 0 || len(address.TransportStyle) > 255 {
			return RouterInfoContacts{}, foundation.NetworkDatabaseErrMalformed
		}
...
func marshalLocalRouterInfo(identity []byte, private ed25519.PrivateKey, published uint64, contacts RouterInfoContacts) ([]byte, error) {
	...
	if n > foundation.NetworkDatabaseMaxRouterInfoBytes {
		return nil, foundation.NetworkDatabaseErrStructureTooLarge
	}
	...
	if offset+signatureLen != len(raw) {
		return nil, foundation.NetworkDatabaseErrMalformed
	}
	return raw, nil
}

func parseOwnedRouterInfo(raw []byte) (foundation.NetworkDatabaseRouterInfo, error) {
	copyRaw := append([]byte(nil), raw...)
	info, err := foundation.NetworkDatabaseParseRouterInfo(copyRaw)
	if err != nil {
		return foundation.NetworkDatabaseRouterInfo{}, err
	}
	return info, nil
