...
type leaseEntry struct {
	legacy    foundation.NetworkDatabaseLeaseSet
	v2        foundation.NetworkDatabaseLeaseSet2
	meta      foundation.NetworkDatabaseMetaLeaseSet
	encrypted foundation.NetworkDatabaseEncryptedLeaseSet
	version   uint64
	expires   uint64
	published bool
...
func RouterInfoFresh(info foundation.NetworkDatabaseRouterInfo, nowMillis uint64) error {
...
func ReseedRouterInfoFresh(info foundation.NetworkDatabaseRouterInfo, nowMillis uint64) error {
	return routerInfoFresh(info, nowMillis, ReseedRouterInfoMaxAgeMillis)
}

func routerInfoFresh(info foundation.NetworkDatabaseRouterInfo, nowMillis, maxAgeMillis uint64) error {
...
func (d *Database) admitRouterInfo(info foundation.NetworkDatabaseRouterInfo, seenAt, maxAgeMillis uint64) error {
	...
	ownedInfo, err := foundation.NetworkDatabaseParseRouterInfo(owned)
	if err != nil {
		return err
	}
...
func (d *Database) HandleDatabaseStore(store foundation.I2NPDatabaseStoreMessage, floodfill bool, seenAt uint64) error {
...
func (d *Database) HandleDatabaseStoreAsPublished(store foundation.I2NPDatabaseStoreMessage, floodfill bool, seenAt uint64, published bool) error {
	var err error
	switch store.Type {
...
func (d *Database) StoreMatchesCurrent(store foundation.I2NPDatabaseStoreMessage) bool {
	if store.Type == foundation.I2NPStoreRouterInfo {
		inflated, lease, err := d.inflateRouterInfo(store.Data)
		if err != nil {
			return false
		}
		defer lease.Release()
		incoming, err := foundation.NetworkDatabaseParseRouterInfo(inflated)
		if err != nil {
			return false
		}
...
func (d *Database) storeRouterInfo(store foundation.I2NPDatabaseStoreMessage, floodfill bool, seenAt uint64) error {
	inflated, lease, err := d.inflateRouterInfo(store.Data)
	if err != nil {
		return err
	}
	defer lease.Release()
	parsed, err := foundation.NetworkDatabaseParseRouterInfo(inflated)
	if err != nil {
		return err
	}
...
func (d *Database) storeLeaseSet(store foundation.I2NPDatabaseStoreMessage, seenAt uint64, published bool) error {
	parsed, err := foundation.NetworkDatabaseParseLeaseSet(store.Data)
	if err != nil {
		return err
	}
	if parsed.Hash() != store.Key {
	...
	parsed, err = foundation.NetworkDatabaseParseLeaseSet(owned)
	if err != nil {
		return err
	}
	d.leasesMu.Lock()
...
func (d *Database) storeLeaseSet2(store foundation.I2NPDatabaseStoreMessage, seenAt uint64, published bool) error {
	parsed, err := foundation.NetworkDatabaseParseLeaseSet2(store.Data)
	if err != nil {
		return err
	}
	if parsed.Hash() != store.Key {
	...
	owned := append([]byte(nil), store.Data...)
	parsed, err = foundation.NetworkDatabaseParseLeaseSet2(owned)
	if err != nil {
		return err
	}
...
func (d *Database) storeMetaLeaseSet(store foundation.I2NPDatabaseStoreMessage, seenAt uint64, published bool) error {
	parsed, err := foundation.NetworkDatabaseParseMetaLeaseSet(store.Data)
	if err != nil {
		return err
	}
	if parsed.Hash() != store.Key {
	...
	owned := append([]byte(nil), store.Data...)
	parsed, err = foundation.NetworkDatabaseParseMetaLeaseSet(owned)
	if err != nil {
		return err
	}
	version := uint64(parsed.Header.Published)
...
func (d *Database) storeEncryptedLeaseSet(store foundation.I2NPDatabaseStoreMessage, seenAt uint64, published bool) error {
	parsed, err := foundation.NetworkDatabaseParseEncryptedLeaseSet(store.Data)
	if err != nil {
		return err
	}
	...
	owned := append([]byte(nil), store.Data...)
	parsed, err = foundation.NetworkDatabaseParseEncryptedLeaseSet(owned)
	if err != nil {
		return err
	}
...
func legacyLeaseRange(set foundation.NetworkDatabaseLeaseSet) (uint64, uint64, error) {
	leases := set.Leases()
...
func leaseSet2Range(set foundation.NetworkDatabaseLeaseSet2) (uint64, uint64, error) {
	leases := set.Leases()
	earliest, latest := ^uint64(0), uint64(0)
	for {
		lease, ok, err := leases.Next()
		...
		if set.Header.Offline.Present() && lease.EndDate > set.Header.Offline.Expires {
			return 0, 0, foundation.NetworkDatabaseErrMalformed
		}
...
func metaLeaseSetRange(set foundation.NetworkDatabaseMetaLeaseSet) (uint64, uint64, error) {
	leases := set.Leases()
	earliest, latest := ^uint64(0), uint64(0)
	for {
		lease, ok, err := leases.Next()
...
func (d *Database) LeaseSet(key foundation.Hash) (foundation.NetworkDatabaseLeaseSet, bool) {
	d.leasesMu.RLock()
	entry, ok := d.leases[key]
	d.leasesMu.RUnlock()
	return entry.legacy, ok && entry.typeID == foundation.I2NPStoreLeaseSet
}

// LeaseSet2 returns the newest LS2 for key, when one is stored.
func (d *Database) LeaseSet2(key foundation.Hash) (foundation.NetworkDatabaseLeaseSet2, bool) {
	d.leasesMu.RLock()
	entry, ok := d.leases[key]
	d.leasesMu.RUnlock()
	return entry.v2, ok && entry.typeID == foundation.I2NPStoreLeaseSet2
}

// MetaLeaseSet returns the newest meta LeaseSet for key, when one is stored.
func (d *Database) MetaLeaseSet(key foundation.Hash) (foundation.NetworkDatabaseMetaLeaseSet, bool) {
	d.leasesMu.RLock()
	entry, ok := d.leases[key]
	d.leasesMu.RUnlock()
	return entry.meta, ok && entry.typeID == foundation.I2NPStoreMetaLeaseSet
}

// EncryptedLeaseSet returns the newest encrypted LeaseSet for key.
func (d *Database) EncryptedLeaseSet(key foundation.Hash) (foundation.NetworkDatabaseEncryptedLeaseSet, bool) {
	d.leasesMu.RLock()
	entry, ok := d.leases[key]
	d.leasesMu.RUnlock()
	return entry.encrypted, ok && entry.typeID == foundation.I2NPStoreEncryptedLeaseSet
}
...
func (d *Database) inflateRouterInfo(compressed []byte) ([]byte, *pool.Lease, error) {
	...
	lease, ok := pool.AcquireLease(foundation.NetworkDatabaseMaxRouterInfoBytes + 1)
	if !ok {
		return nil, nil, ErrRouterInfoTooLarge
	}
	output, _ := lease.Bytes(foundation.NetworkDatabaseMaxRouterInfoBytes + 1)
	used := 0
	for {
		n, readErr := reader.Read(output[used:])
		used += n
		if used > foundation.NetworkDatabaseMaxRouterInfoBytes {
			reader.Close()
