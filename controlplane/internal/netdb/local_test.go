...
func TestLocalLeaseSetSnapshotExpiresAndReplaces(t *testing.T) {
	...
	if err := local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{TunnelID: 3, EndDate: 30}}); err != nil {
		t.Fatal(err)
	}
...
func TestLocalLeaseSetCopiesIdentityAndBoundsLeases(t *testing.T) {
	...
	tooMany := make([]foundation.NetworkDatabaseLease, foundation.NetworkDatabaseMaxLeases+1)
	for index := range tooMany {
		tooMany[index].TunnelID = uint32(index + 1)
	}
	if err := local.ReplaceInboundLeases(tooMany); !errors.Is(err, foundation.NetworkDatabaseErrTooManyItems) {
	...
	if err := local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{TunnelID: 0, EndDate: 1}}); !errors.Is(err, foundation.NetworkDatabaseErrMalformed) {
...
func TestLocalLeaseSetSnapshotMarshalLegacy(t *testing.T) {
	...
	gateway[0] = 1
	if err := local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{Gateway: gateway, TunnelID: 1, EndDate: 2}}); err != nil {
		t.Fatal(err)
	}
	snapshot, ok := local.Snapshot(0)
	if !ok {
	...
	parsed, err := foundation.NetworkDatabaseParseLeaseSet(wireBytes[:n])
	if err != nil {
		t.Fatal(err)
	}
...
func TestLocalLeaseSetSnapshotMarshalLegacyRejectsMalformedKeysAndSignature(t *testing.T) {
	...
	if _, err := snapshot.MarshalLegacy(wireBytes, encryptionKey[:255], signingKey, signer); !errors.Is(err, foundation.NetworkDatabaseErrInvalidKeyLength) {
		t.Fatalf("short encryption key error = %v, want ErrInvalidKeyLength", err)
	}
	if _, err := snapshot.MarshalLegacy(wireBytes, encryptionKey, signingKey[:31], signer); !errors.Is(err, foundation.NetworkDatabaseErrInvalidKeyLength) {
