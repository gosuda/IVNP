...
func (s LocalLeaseSetSnapshot) MarshalLegacy(dst, encryptionKey, signingKey []byte, sign func([]byte) ([]byte, error)) (int, error) {
	...
	if identityLen != len(identityBytes) {
		return 0, foundation.NetworkDatabaseErrMalformed
	}
	if len(encryptionKey) != 256 {
		return 0, foundation.NetworkDatabaseErrInvalidKeyLength
	}
	...
	if len(s.Leases) > foundation.NetworkDatabaseMaxLeases {
		return 0, foundation.NetworkDatabaseErrTooManyItems
	}
	...
	if totalLen > foundation.NetworkDatabaseMaxLeaseSetBytes {
		return 0, foundation.NetworkDatabaseErrStructureTooLarge
	}
	if len(dst) < totalLen {
...
func (s *LocalLeaseSet) ReplaceInboundLeases(leases []foundation.NetworkDatabaseLease) error {
	if len(leases) > foundation.NetworkDatabaseMaxLeases {
		return foundation.NetworkDatabaseErrTooManyItems
	}
	for _, lease := range leases {
		if lease.TunnelID == 0 {
			return foundation.NetworkDatabaseErrMalformed
		}
	}
...
func (s *LocalLeaseSet) Snapshot(nowMillis uint64) (LocalLeaseSetSnapshot, bool) {
	...
	leases := make([]foundation.NetworkDatabaseLease, 0, len(s.leases))
