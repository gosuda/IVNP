...
func (s *LocalLeaseSet2) ReplaceInboundLeases(leases []foundation.NetworkDatabaseLease) error {
	if s == nil || len(leases) > foundation.NetworkDatabaseMaxLeases {
		return ErrLocalLeaseSet2
	}
	converted := make([]foundation.NetworkDatabaseLease2, 0, len(leases))
	for _, lease := range leases {
		if lease.TunnelID == 0 || lease.EndDate/1000 > uint64(^uint32(0)) {
			return ErrLocalLeaseSet2
		}
		converted = append(converted, foundation.NetworkDatabaseLease2{Gateway: lease.Gateway, TunnelID: lease.TunnelID, EndDate: uint32(lease.EndDate / 1000)})
	}
	s.mu.Lock()
	s.leases = converted
	s.mu.Unlock()
...
func (s *LocalLeaseSet2) MarshalTo(dst []byte, nowMillis uint64, sign func([]byte) ([]byte, error)) (int, error) {
	...
	if offline != nil {
		...
		flags = foundation.NetworkDatabaseLeaseSetOfflineFlag
	...
	signed[0] = byte(foundation.I2NPStoreLeaseSet2)
