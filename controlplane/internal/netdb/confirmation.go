...
type ConfirmedPublisher interface {
	...
	HandleDeliveryStatus(foundation.I2NPDeliveryStatusMessage) bool
}

type publicationTokenOwner func(foundation.I2NPDeliveryStatusMessage) bool
...
func (p *confirmedPublication) maintain(ctx context.Context, force bool) (int, error) {
	...
	for !stop {
		...
		for len(batch) < PublicationFloodfillK {
			...
			payload, err := foundation.NetworkDatabaseMarshalDatabaseStore(p.key, p.typeID, data, token, gateway, tunnelID)
			if err != nil {
				p.registry.retire(token)
				if first == nil {
					first = err
			...
			message := foundation.I2NPMessage{Header: foundation.I2NPHeader{Type: foundation.I2NPDatabaseStore, ID: p.messageID(), Expiration: saturatingAdd(now, leaseSetPublicationEnvelopeLifetime)}, Payload: payload}
			p.mu.Lock()
...
func (p *confirmedPublication) handle(status foundation.I2NPDeliveryStatusMessage) bool {
