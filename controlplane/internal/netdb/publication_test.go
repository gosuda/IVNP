...
// SetOptions carries extension entries into the published record, and an
// option change within the same published second still produces a strictly
// newer record — the store only replaces on a newer version.
func TestLeaseSetPublisherPublishesExtensionOptions(t *testing.T) {
	...
	publisher, err := NewLeaseSetPublisher(LeaseSetPublisherConfig{
		Local2: local, Database: database, Sender: sender,
		InboundLeases: &publisherLeaseSource{leases: []foundation.NetworkDatabaseLease{{TunnelID: 9, EndDate: now + 60_000}}},
	...
	first := publishedLS2(t, sender.published[0].message.Payload)

	if err := publisher.SetOptions([]foundation.MappingEntry{
	...
	second := publishedLS2(t, sender.published[1].message.Payload)
	if second.Header.Published <= first.Header.Published {
		t.Fatalf("changed record version did not advance: %d <= %d", second.Header.Published, first.Header.Published)
	}
	it := second.Options.Iterator()
	key, value, ok, err := it.Next()
	if err != nil || !ok || string(key) != "x-ivnp.v" || string(value) != "1" {
		t.Fatalf("published record options start = %q=%q, %t, %v", key, value, ok, err)
	}

	// A closed publisher and an encrypted publisher both reject options.
	publisher.Close()
	if err := publisher.SetOptions(nil); err == nil {
		t.Fatal("closed publisher accepted options")
	}
	...
	encryptedPublisher, err := NewLeaseSetPublisher(LeaseSetPublisherConfig{
		Encrypted: encrypted, Database: database, Sender: &publisherSender{},
		InboundLeases: &publisherLeaseSource{leases: []foundation.NetworkDatabaseLease{{TunnelID: 9, EndDate: now + 60_000}}},
	...
	defer encryptedPublisher.Close()
	if err := encryptedPublisher.SetOptions([]foundation.MappingEntry{
		{Key: []byte("x-ivnp.v"), Value: []byte("1")},
	}); err == nil {
		t.Fatal("encrypted publisher accepted cleartext options")
	}
}

func publishedLS2(t *testing.T, payload []byte) foundation.NetworkDatabaseLeaseSet2 {
	t.Helper()
