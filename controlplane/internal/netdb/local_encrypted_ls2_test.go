...
func encryptedTestSet(t *testing.T, auth EncryptedLeaseSetAuthorization) (*foundation.LocalDestination, foundation.Identity, *LocalEncryptedLeaseSet, uint64) {
	...
	if err = local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{Gateway: foundation.Hash{1}, TunnelID: 7, EndDate: now + 3_600_000}}); err != nil {
		t.Fatal(err)
	}
...
func TestEncryptedLeaseSetRoundTripAndTampering(t *testing.T) {
	...
	set, err := foundation.NetworkDatabaseParseEncryptedLeaseSet(payload[:n])
	if err != nil {
		t.Fatal(err)
	}
	...
	bad, err := foundation.NetworkDatabaseParseEncryptedLeaseSet(tampered)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLocalEncryptedLeaseSetWriterDoesNotApplyReaderCap(t *testing.T) {
	...
	if encryptedLength <= foundation.NetworkDatabaseMaxEncryptedLeaseSetDataBytes {
		t.Fatalf("Java-compatible writer encrypted data = %d bytes, want over reader cap %d", encryptedLength, foundation.NetworkDatabaseMaxEncryptedLeaseSetDataBytes)
	}
	if _, err = foundation.NetworkDatabaseParseEncryptedLeaseSet(payload[:n]); !errors.Is(err, foundation.NetworkDatabaseErrMalformed) {
...
func TestLocalEncryptedLeaseSetFailureCleanupAndRelease(t *testing.T) {
	...
	dst := make([]byte, foundation.NetworkDatabaseMaxLeaseSetBytes)
	for index := range dst {
		dst[index] = 0xa5
	}
...
func TestLeaseSetPublisherCloseReleasesSensitiveOwners(t *testing.T) {
	...
	publisher, err := NewLeaseSetPublisher(LeaseSetPublisherConfig{
		...
		InboundLeases: InboundLeaseSourceFunc(func(uint64) []foundation.NetworkDatabaseLease { return nil }),
		Sender:        LeaseSetPublishSenderFunc(func(context.Context, RouterRef, foundation.I2NPMessage) error { return nil }),
...
func TestEncryptedLeaseSetPublisherUsesConfirmedStoreType(t *testing.T) {
	...
	publisher, err := NewLeaseSetPublisher(LeaseSetPublisherConfig{
		...
		InboundLeases: InboundLeaseSourceFunc(func(uint64) []foundation.NetworkDatabaseLease {
			return []foundation.NetworkDatabaseLease{{Gateway: foundation.Hash{1}, TunnelID: 7, EndDate: now + 10*60_000}}
		}),
		Sender:         LeaseSetPublishSenderFunc(func(_ context.Context, _ RouterRef, _ foundation.I2NPMessage) error { return nil }),
