...
func TestLocalLeaseSet2OfflineSignatureSection(t *testing.T) {
	...
	if set.Header.Flags&foundation.NetworkDatabaseLeaseSetOfflineFlag == 0 || !set.Header.Offline.Present() {
...
func TestLocalLeaseSet2OfflineCapsLeaseExpiry(t *testing.T) {
	...
	var gateway foundation.Hash
	gateway[0] = 1
	if err = local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{Gateway: gateway, TunnelID: 7, EndDate: now + 3_600_000}}); err != nil {
		t.Fatal(err)
	}
	payload := make([]byte, foundation.NetworkDatabaseMaxLeaseSetBytes)
	n, err := local.MarshalTo(payload, now, destination.Sign)
...
func TestOfflineLeaseSetExpiryHasNoClockFudge(t *testing.T) {
	...
	var gateway foundation.Hash
	gateway[0] = 1
	if err = local.ReplaceInboundLeases([]foundation.NetworkDatabaseLease{{Gateway: gateway, TunnelID: 7, EndDate: uint64(expires) * 1000}}); err != nil {
		t.Fatal(err)
	}
	payload := make([]byte, foundation.NetworkDatabaseMaxLeaseSetBytes)
	n, err := local.MarshalTo(payload, now, destination.Sign)
	...
	store := foundation.I2NPDatabaseStoreMessage{Key: destination.Hash(), Type: foundation.I2NPStoreLeaseSet2, Data: payload[:n]}
