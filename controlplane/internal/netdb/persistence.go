...
func (s *RouterInfoStore) MaxBytes() int {
	return routerSnapshotHeader + s.max*(8+2+foundation.NetworkDatabaseMaxRouterInfoBytes+routerSnapshotDigest)
}
...
func (s *RouterInfoStore) loadLocked(data []byte, nowMillis uint64) (RouterInfoLoadResult, error) {
	...
	for range count {
		...
		cursor += 10
		if wireLen == 0 || wireLen > foundation.NetworkDatabaseMaxRouterInfoBytes || len(data)-cursor < wireLen+routerSnapshotDigest {
			return result, ErrRouterSnapshotRecord
		}
		...
		info, parseErr := foundation.NetworkDatabaseParseRouterInfo(wire)
		if parseErr != nil {
			result.Rejected++
			continue
		}
...
func (s *RouterInfoStore) encode(peers []RouterRef) ([]byte, error) {
	...
	for _, peer := range peers {
		...
		if len(wire) == 0 || len(wire) > foundation.NetworkDatabaseMaxRouterInfoBytes || len(wire) > int(^uint16(0)) {
