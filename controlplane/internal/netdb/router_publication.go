...
func (p *RouterInfoPublisher) Maintain(ctx context.Context) (int, error) {
	...
	if string(p.last) != string(raw) {
		compressed, err := foundation.NetworkDatabaseCompressRouterInfo(raw)
		if err != nil {
			return 0, err
		}
...
func (p *RouterInfoPublisher) HandleDeliveryStatus(status foundation.I2NPDeliveryStatusMessage) bool {
