...
type storeFloodJob struct {
	...
	store  foundation.I2NPDatabaseStoreMessage
}
...
func (f *StoreFlooder) Enqueue(source foundation.Hash, store foundation.I2NPDatabaseStoreMessage) error {
...
func (f *StoreFlooder) flood(ctx context.Context, job *storeFloodJob) {
	...
	payload, err := foundation.NetworkDatabaseMarshalDatabaseStore(job.store.Key, job.store.Type, job.store.Data, 0, foundation.Hash{}, 0)
	if err != nil {
	...
	message := foundation.I2NPMessage{
		Header:  foundation.I2NPHeader{Type: foundation.I2NPDatabaseStore, ID: messageID, Expiration: saturatingAdd(now, storeFlooderEnvelopeLifetime)},
		Payload: payload,
	}
...
func (f *StoreFlooder) markNew(store foundation.I2NPDatabaseStoreMessage, now uint64) bool {
	hash := sha256.New()
...
func (f *StoreFlooder) floodCandidates(store foundation.I2NPDatabaseStoreMessage, now uint64) []RouterRef {
...
func storeNeedsNextRoutingKey(store foundation.I2NPDatabaseStoreMessage, now uint64) bool {
	until := timeUntilUTCMidnight(now)
	...
	case foundation.I2NPStoreLeaseSet:
		var set foundation.NetworkDatabaseLeaseSet
		set, err = foundation.NetworkDatabaseParseLeaseSet(store.Data)
		if err == nil {
			_, latest, err = legacyLeaseRange(set)
		}
	case foundation.I2NPStoreLeaseSet2:
		var set foundation.NetworkDatabaseLeaseSet2
	...
	case foundation.I2NPStoreMetaLeaseSet:
		var set foundation.NetworkDatabaseMetaLeaseSet
		set, err = foundation.NetworkDatabaseParseMetaLeaseSet(store.Data)
		if err == nil {
			_, latest, err = metaLeaseSetRange(set)
		}
	case foundation.I2NPStoreEncryptedLeaseSet:
		var set foundation.NetworkDatabaseEncryptedLeaseSet
		set, err = foundation.NetworkDatabaseParseEncryptedLeaseSet(store.Data)
		if err == nil {
